...
new WellCMSNotification()
    ...
    .body('Changes to the post have been saved.')
    .actions([
        new WellCMSNotificationAction('view')
            .button(),
        new WellCMSNotificationAction('undo')
            .color('gray'),
    ])
    .send()
```
...
The JavaScript objects (`WellCMSNotification` and `WellCMSNotificationAction`) are assigned to `window.WellCMSNotification` and `window.WellCMSNotificationAction`, so they are available in on-page scripts.

You may also import them in a bundled JavaScript file:

```js
import { Notification, NotificationAction } from '../../vendor/wellcms/notifications/dist/index.js'
