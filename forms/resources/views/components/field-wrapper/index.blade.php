...
@php
    if ($field) {
        ...
        $hasNestedRecursiveValidationRules ??= $field instanceof \WellCMS\Forms\Components\Contracts\HasNestedRecursiveValidationRules;
    ...
    $hintActions = array_filter(
        $hintActions ?? [],
        fn (\WellCMS\Forms\Components\Actions\Action $hintAction): bool => $hintAction->isVisible(),
    );
...
>
    ...
    >
        @if (($label && (! $labelSrOnly)) || $labelPrefix || $labelSuffix || filled($hint) || $hintIcon || count($hintActions))
            ...
            >
                @if ($label && (! $labelSrOnly))
                    <x-wellcms-forms::field-wrapper.label
                ...
                @if (filled($hint) || $hintIcon || count($hintActions))
                    <x-wellcms-forms::field-wrapper.hint
                        :actions="$hintActions"
                        :color="$hintColor"
                        :icon="$hintIcon"
        ...
        @if ((! \WellCMS\Support\is_slot_empty($slot)) || $hasError || filled($helperText))
            <div
                @class([
            ...
            >
                ...
                @if ($hasError)
                    <x-wellcms-forms::field-wrapper.error-message>
                ...
                @if (filled($helperText))
                    <x-wellcms-forms::field-wrapper.helper-text>
                        {{ $helperText }}
                    </x-wellcms-forms::field-wrapper.helper-text>
                @endif
            </div>
