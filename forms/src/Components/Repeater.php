...
class Repeater extends Field implements Contracts\CanConcealComponents, Contracts\HasExtraItemActions
{
    ...
    {
        return $this->evaluate($this->addBetweenActionLabel) ?? __('wellcms-forms::components.repeater.actions.add_between.label');
    }

    public function getCloneAction(): Action
    {
        $action = Action::make($this->getCloneActionName())
            ->label(__('wellcms-forms::components.repeater.actions.clone.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.clone') ?? 'heroicon-m-square-2-stack')
            ->color('gray')
            ->action(function (array $arguments, Repeater $component): void {
                $newUuid = $component->generateUuid();
    ...
    public function getDeleteAction(): Action
    {
        $action = Action::make($this->getDeleteActionName())
            ->label(__('wellcms-forms::components.repeater.actions.delete.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.delete') ?? 'heroicon-m-trash')
            ->color('danger')
            ->action(function (array $arguments, Repeater $component): void {
                $items = $component->getState();
    ...
    public function getMoveDownAction(): Action
    {
        $action = Action::make($this->getMoveDownActionName())
            ->label(__('wellcms-forms::components.repeater.actions.move_down.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.move-down') ?? 'heroicon-m-arrow-down')
            ->color('gray')
            ->action(function (array $arguments, Repeater $component): void {
    ...
    public function getMoveUpAction(): Action
    {
        $action = Action::make($this->getMoveUpActionName())
            ->label(__('wellcms-forms::components.repeater.actions.move_up.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.move-up') ?? 'heroicon-m-arrow-up')
            ->color('gray')
            ->action(function (array $arguments, Repeater $component): void {
    ...
    public function getReorderAction(): Action
    {
        $action = Action::make($this->getReorderActionName())
            ->label(__('wellcms-forms::components.repeater.actions.reorder.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.reorder') ?? 'heroicon-m-arrows-up-down')
            ->color('gray')
            ->action(function (array $arguments, Repeater $component): void {
                $items = [
    ...
    public function getCollapseAction(): Action
    {
        $action = Action::make($this->getCollapseActionName())
            ->label(__('wellcms-forms::components.repeater.actions.collapse.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.collapse') ?? 'heroicon-m-chevron-up')
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->iconButton()
            ->size(ActionSize::Small);
    ...
    public function getExpandAction(): Action
    {
        $action = Action::make($this->getExpandActionName())
            ->label(__('wellcms-forms::components.repeater.actions.expand.label'))
            ->icon(WellCMSIcon::resolve('forms::components.repeater.actions.expand') ?? 'heroicon-m-chevron-down')
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->iconButton()
            ->size(ActionSize::Small);
    ...
    public function getCollapseAllAction(): Action
    {
        $action = Action::make($this->getCollapseAllActionName())
            ->label(__('wellcms-forms::components.repeater.actions.collapse_all.label'))
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->link()
            ->size(ActionSize::Small);
    ...
    public function getExpandAllAction(): Action
    {
        $action = Action::make($this->getExpandAllActionName())
            ->label(__('wellcms-forms::components.repeater.actions.expand_all.label'))
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->link()
            ->size(ActionSize::Small);
    ...
    {
        return $this->evaluate($this->addActionLabel) ?? __('wellcms-forms::components.repeater.actions.add.label', [
    ...
    public function relationship(string | Closure | null $name = null, ?Closure $modifyQueryUsing = null): static
    {
        ...
        $this->saveRelationshipsUsing(static function (Repeater $component, HasForms $livewire, ?array $state) {
            ...
            $translatableContentDriver = $livewire->makeWellCMSTranslatableContentDriver();
    ...
    protected function getStateFromRelatedRecords(Collection $records): array
    {
        ...
        $translatableContentDriver = $this->getLivewire()->makeWellCMSTranslatableContentDriver();
    ...
    {
        if ($this->isSimple()) {
            return 'wellcms-forms::components.repeater.simple';
        }

        return 'wellcms-forms::components.repeater.index';
    }
