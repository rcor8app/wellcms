...
use Closure;
use WellCMS\Forms\ComponentContainer;
use WellCMS\Forms\Components\Actions\Action;
use WellCMS\Forms\Components\Builder\Block;
use WellCMS\Support\Concerns\HasReorderAnimationDuration;
...
use function WellCMS\Forms\array_move_after;
use function WellCMS\Forms\array_move_before;
...
class Builder extends Field implements Contracts\CanConcealComponents, Contracts\HasExtraItemActions
{
     ...
     * @var view-string
     */
    protected string $view = 'wellcms-forms::components.builder';
    ...
    public function getAddAction(): Action
    {
        ...
        if ($this->hasBlockPreviews()) {
            $action
                ->modalHeading(fn (Builder $component) => __('wellcms-forms::components.builder.actions.add.modal.heading', [
                    'label' => $component->getLabel(),
                ]))
                ->modalSubmitActionLabel(__('wellcms-forms::components.builder.actions.add.modal.actions.add.label'))
    ...
    public function getAddBetweenAction(): Action
    {
        ...
        if ($this->hasBlockPreviews()) {
            $action
                ->modalHeading(fn (Builder $component) => __('wellcms-forms::components.builder.actions.add_between.modal.heading', [
                    'label' => $component->getLabel(),
                ]))
                ->modalSubmitActionLabel(__('wellcms-forms::components.builder.actions.add_between.modal.actions.add.label'))
                ->form(function (array $arguments, Builder $component): array {
                    return $component->getBlock($arguments['block'])->getChildComponents();
                });
        }
    ...
    public function getCloneAction(): Action
    {
        $action = Action::make($this->getCloneActionName())
            ->label(__('wellcms-forms::components.builder.actions.clone.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.clone') ?? 'heroicon-m-square-2-stack')
            ->color('gray')
    ...
    public function getDeleteAction(): Action
    {
        $action = Action::make($this->getDeleteActionName())
            ->label(__('wellcms-forms::components.builder.actions.delete.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.delete') ?? 'heroicon-m-trash')
            ->color('danger')
            ->action(function (array $arguments, Builder $component): void {
                $items = $component->getState();
    ...
    public function getMoveDownAction(): Action
    {
        $action = Action::make($this->getMoveDownActionName())
            ->label(__('wellcms-forms::components.builder.actions.move_down.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.move-down') ?? 'heroicon-m-arrow-down')
            ->color('gray')
            ->action(function (array $arguments, Builder $component): void {
    ...
    public function getMoveUpAction(): Action
    {
        $action = Action::make($this->getMoveUpActionName())
            ->label(__('wellcms-forms::components.builder.actions.move_up.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.move-up') ?? 'heroicon-m-arrow-up')
            ->color('gray')
            ->action(function (array $arguments, Builder $component): void {
    ...
    public function getReorderAction(): Action
    {
        $action = Action::make($this->getReorderActionName())
            ->label(__('wellcms-forms::components.builder.actions.reorder.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.reorder') ?? 'heroicon-m-arrows-up-down')
            ->color('gray')
            ->action(function (array $arguments, Builder $component): void {
                $items = [
    ...
    public function getCollapseAction(): Action
    {
        $action = Action::make($this->getCollapseActionName())
            ->label(__('wellcms-forms::components.builder.actions.collapse.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.collapse') ?? 'heroicon-m-chevron-up')
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->iconButton()
            ->size(ActionSize::Small);
    ...
    public function getExpandAction(): Action
    {
        $action = Action::make($this->getExpandActionName())
            ->label(__('wellcms-forms::components.builder.actions.expand.label'))
            ->icon(WellCMSIcon::resolve('forms::components.builder.actions.expand') ?? 'heroicon-m-chevron-down')
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->iconButton()
            ->size(ActionSize::Small);
    ...
    public function getCollapseAllAction(): Action
    {
        $action = Action::make($this->getCollapseAllActionName())
            ->label(__('wellcms-forms::components.builder.actions.collapse_all.label'))
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->link()
            ->size(ActionSize::Small);
    ...
    public function getExpandAllAction(): Action
    {
        $action = Action::make($this->getExpandAllActionName())
            ->label(__('wellcms-forms::components.builder.actions.expand_all.label'))
            ->color('gray')
            ->livewireClickHandlerEnabled(false)
            ->link()
            ->size(ActionSize::Small);
    ...
    public function getEditAction(): Action
    {
        $action = Action::make($this->getEditActionName())
            ->label(__('wellcms-forms::components.builder.actions.edit.label'))
            ->modalHeading(__('wellcms-forms::components.builder.actions.edit.modal.heading'))
            ->modalSubmitActionLabel(__('wellcms-forms::components.builder.actions.edit.modal.actions.save.label'))
            ->color('gray')
    ...
    {
        return $this->evaluate($this->addBetweenActionLabel) ?? __('wellcms-forms::components.builder.actions.add_between.label');
    }

    public function getAddActionLabel(): string
    {
        return $this->evaluate($this->addActionLabel) ?? __('wellcms-forms::components.builder.actions.add.label', [
