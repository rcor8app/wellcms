...
use Closure;
use WellCMS\Forms\Components\Contracts\CanBeLengthConstrained;
use WellCMS\Forms\Components\Contracts\HasNestedRecursiveValidationRules;
            ...
            return function (string $attribute, mixed $value, Closure $fail) use ($component, $state) {
                ...
                if (is_bool($state)) {
                    ...
                    if ($state && $isSiblingItemSelected) {
                        $fail(__($validationMessages['distinct.only_one_must_be_selected'] ?? 'wellcms-forms::validation.distinct.only_one_must_be_selected', ['attribute' => $component->getValidationAttribute()]));
                    ...
                    $fail(__($validationMessages['distinct.must_be_selected'] ?? 'wellcms-forms::validation.distinct.must_be_selected', ['attribute' => $component->getValidationAttribute()]));
