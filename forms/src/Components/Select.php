...
use WellCMS\Support\Services\RelationshipJoiner;
...
use function WellCMS\Support\generate_search_column_expression;
use function WellCMS\Support\generate_search_term_expression;
...
class Select extends Field implements Contracts\CanDisableOptions, Contracts\HasAffixActions, Contracts\HasNestedRecursiveValidationRules
{
     ...
     * @var view-string
     */
    protected string $view = 'wellcms-forms::components.select';
    ...
    protected function setUp(): void
    {
        ...
        $this->placeholder(static fn (Select $component): ?string => $component->isDisabled() ? null : __('wellcms-forms::components.select.placeholder'));
    ...
    {
        $this->options([
            1 => $trueLabel ?? __('wellcms-forms::components.select.boolean.true'),
            0 => $falseLabel ?? __('wellcms-forms::components.select.boolean.false'),
        ]);
    ...
    public function getCreateOptionAction(): ?Action
    {
        ...
        $action = Action::make($this->getCreateOptionActionName())
            ->label(__('wellcms-forms::components.select.actions.create_option.label'))
            ...
            ->color('gray')
            ->icon(WellCMSIcon::resolve('forms::components.select.actions.create-option') ?? 'heroicon-m-plus')
            ->iconButton()
            ->modalHeading($this->getCreateOptionModalHeading() ?? __('wellcms-forms::components.select.actions.create_option.modal.heading'))
            ->modalSubmitActionLabel(__('wellcms-forms::components.select.actions.create_option.modal.actions.create.label'))
            ->extraModalFooterActions(fn (Action $action, Select $component): array => $component->isMultiple() ? [
                $action->makeModalSubmitAction('createAnother', arguments: ['another' => true])
                    ->label(__('wellcms-forms::components.select.actions.create_option.modal.actions.create_another.label')),
            ] : []);
    ...
    public function getEditOptionAction(): ?Action
    {
        ...
        $action = Action::make($this->getEditOptionActionName())
            ->label(__('wellcms-forms::components.select.actions.edit_option.label'))
            ->form(static function (Select $component, Form $form): array | Form | null {
            ...
            ->color('gray')
            ->icon(WellCMSIcon::resolve('forms::components.select.actions.edit-option') ?? 'heroicon-m-pencil-square')
            ->iconButton()
            ->modalHeading($this->getEditOptionModalHeading() ?? __('wellcms-forms::components.select.actions.edit_option.modal.heading'))
            ->modalSubmitActionLabel(__('wellcms-forms::components.select.actions.edit_option.modal.actions.save.label'))
        ...
        $this->loadStateFromRelationshipsUsing(static function (Select $component, $state) use ($modifyQueryUsing): void {
            ...
            if (
                ...
                $component->state(
                    ...
                    //
                    // https://github.com/wellcmsphp/wellcms/issues/1111
                    $relatedRecords
    ...
    public function getMaxItemsMessage(): string
    {
        ...
        return $this->evaluate($this->maxItemsMessage) ?? trans_choice('wellcms-forms::components.select.max_items_message', $maxItems, [
            ':count' => $maxItems,
        ]);
    }
    ...
    public function refreshSelectedOptionLabel(): void
    {
        ...
        $livewire->dispatch(
            'wellcms-forms::select.refreshSelectedOptionLabel',
