...
You can return an array from the function if you want WellCMS to continue to assert the form state after the function has been run.
...
it('links to the correct WellCMS sites', function () {
    $invoice = Invoice::factory()->create();

    livewire(EditInvoice::class, [
        'invoice' => $invoice,
    ])
        ->assertFormComponentActionHasUrl('customer_id', 'wellcms', 'https://wellcmsphp.com/')
        ->assertFormComponentActionDoesNotHaveUrl('customer_id', 'wellcms', 'https://github.com/wellcmsphp/wellcms')
        ->assertFormComponentActionShouldOpenUrlInNewTab('customer_id', 'wellcms')
