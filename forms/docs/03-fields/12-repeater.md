...
```php
use WellCMS\Forms\Components\Repeater;
...
You may employ the `relationship()` method of the `Repeater` to configure a `HasMany` relationship. WellCMS will load the item data from the relationship, and save it back to the relationship when the form is submitted. If a custom relationship name is not passed to `relationship()`, WellCMS will use the field name as the relationship name:
...
> Please ensure that your pivot model has a primary key column, like `id`, to allow WellCMS to keep track of which repeater items have been created, updated and deleted. To make sure that WellCMS keeps track of the primary key, the pivot model needs to have the `$incrementing` property set to `true`.
...
- If the field returns a boolean, like a [checkbox](checkbox) or [toggle](toggle), and one of the fields is enabled, WellCMS will automatically disable all other enabled fields on behalf of the user.
- Otherwise, for fields like a [select](select), [radio](radio), [checkbox list](checkbox-list), or [toggle buttons](toggle-buttons), when a user selects an option, WellCMS will automatically deselect all other usages of that option on behalf of the user.
