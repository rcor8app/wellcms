...
<LaracastsBanner
    ...
    description="Watch the Build Advanced Components for WellCMS series on Laracasts - it will teach you how to build components, and you'll get to know all the internal tools to help you."
    url="https://laracasts.com/series/build-advanced-components-for-wellcms/episodes/6"
...
```php
use WellCMS\Forms\Components\ViewField;

ViewField::make('rating')
    ->view('wellcms.forms.components.range-slider')
```

This assumes that you have a `resources/views/wellcms/forms/components/range-slider.blade.php` file.
...
{
    protected string $view = 'wellcms.forms.components.range-slider';
}
```

It will also create a view file at `resources/views/wellcms/forms/components/range-slider.blade.php`.
...
If your component heavily relies on third party libraries, we advise that you asynchronously load the Alpine.js component using the WellCMS asset system. This ensures that the Alpine.js component is only loaded when it's needed, and not on every page load. To find out how to do this, check out our [Assets documentation](../../support/assets#asynchronous-alpinejs-components).

## Rendering the field wrapper

WellCMS includes a "field wrapper" component, which is able to render the field's label, validation errors, and any other text surrounding the field. You may render the field wrapper like this in the view:
...
WellCMS provides a `$applyStateBindingModifiers()` function that you may use in your view to apply any state binding modifiers to a `wire:model` or `$wire.$entangle()` binding:
