...
You may pass multiple affix actions to a field by passing them in an array to either `prefixActions()` or `suffixActions()`. Either method can be used, or both at once, WellCMS will render all the registered actions in order:
...
You may pass multiple hint actions to a field by passing them in an array to `hintActions()`. WellCMS will render all the registered actions in order:
...
use App\Actions\ResetStars;
use WellCMS\Forms\Components\Actions;
use WellCMS\Forms\Components\Actions\Action;
