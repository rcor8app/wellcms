...
WellCMS uses CSS variables to define its color palette. These CSS variables are mapped to Tailwind classes in the preset file that you load when installing WellCMS.

## Customizing the default colors

From a service provider's `boot()` method, or middleware, you can call the `WellCMSColor::register()` method, which you can use to customize which colors WellCMS uses for UI elements.

There are 6 default colors that are used throughout WellCMS that you are able to customize:

```php
use WellCMS\Support\Colors\Color;
use WellCMS\Support\Facades\WellCMSColor;

WellCMSColor::register([
    'danger' => Color::Red,
    'gray' => Color::Zinc,
    'info' => Color::Blue,
    'primary' => Color::Amber,
...
You can register extra colors that you can use throughout WellCMS:
