...
WellCMS uses CSS "hook" classes to allow various HTML elements to be customized using CSS.

## Discovering hook classes

We could document all the hook classes across the entire WellCMS UI, but that would be a lot of work, and probably not very useful to you. Instead, we recommend using your browser's developer tools to inspect the elements you want to customize, and then use the hook classes to target those elements.
...
Alternatively, since WellCMS is built upon Tailwind CSS, you can use their `@apply` directive to apply Tailwind classes to WellCMS elements:
...
If you do decide to publish the Blade views, please lock all WellCMS packages to a specific version in your `composer.json` file, and then update WellCMS manually by bumping this number, testing your entire application after each update. This will help you identify breaking changes safely.
