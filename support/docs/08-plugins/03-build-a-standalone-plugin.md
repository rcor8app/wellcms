...
Normally, WellCMS v3 recommends that users style their plugins with a custom wellcms theme, but for the sake of example let's provide our own stylesheet that can be loaded asynchronously using the new `x-load` features in WellCMS v3. So, let's update our `package.json` file to include cssnano, postcss, postcss-cli and postcss-nesting to build our stylesheet.
...
We need to be able to register our stylesheet with the WellCMS Asset Manager so that we can load it on demand in our blade view. To do this, we'll need to add the following to the `packageBooted` method in our service provider.

***Note the `loadedOnRequest()` method. This is important, because it tells WellCMS to only load the stylesheet when it's needed.***
...
use WellCMS\Support\Concerns\HasColor;
...
<{{ $level }}
    x-data
    x-load-css="[@js(\WellCMS\Support\Facades\WellCMSAsset::getStyleHref('headings', package: 'awcodes/headings'))]"
    {{
        $attributes
            ->class([
                'headings-component',
            ...
            ->style([
                \WellCMS\Support\get_color_css_variables($color, [500]) => $color !== 'gray',
            ])
    }}
>
    {{ $getContent() }}
