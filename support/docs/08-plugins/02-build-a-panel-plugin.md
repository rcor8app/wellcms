...
Since WellCMS v3 recommends that users style their plugins with a custom wellcms theme, we'll remove the files needed for using css in the plugin. This is optional, and you can still use css if you want, but it is not recommended.
...
Now we can create our widget. We'll first need to extend WellCMS's `Widget` class in our `ClockWidget.php` file and tell it where to find the view for the widget. Since we are using the PackageServiceProvider to register our views, we can use the `::` syntax to tell WellCMS where to find the view.
...
Next, we'll need to create the view for our widget. Create a new file at `resources/views/widget.blade.php` and add the following code. We'll make use of WellCMS's blade components to save time on writing the html for the widget.
...
<x-wellcms-widgets::widget>
    <x-wellcms::section>
        ...
        <div
            x-load
            x-load-src="{{ \WellCMS\Support\Facades\WellCMSAsset::getAlpineComponentSrc('clock-widget', 'awcodes/clock-widget') }}"
            x-data="clockWidget()"
            class="text-center"
        >
