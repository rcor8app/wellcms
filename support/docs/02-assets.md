...
<LaracastsBanner
    ...
    description="Watch the Build Advanced Components for WellCMS series on Laracasts - it will teach you how to get started with registering assets into a plugin. Alternatively, continue reading this text-based guide."
    url="https://laracasts.com/series/build-advanced-components-for-wellcms/episodes/14"
    series="building-advanced-components"
/>

## Overview

All packages in the WellCMS ecosystem share an asset management system. This allows both official plugins and third-party plugins to register CSS and JavaScript files that can then be consumed by Blade views.

## The `WellCMSAsset` facade

The `WellCMSAsset` facade is used to register files into the asset system. These files may be sourced from anywhere in the filesystem, but are then copied into the `/public` directory of the application when the `php artisan wellcms:assets` command is run. By copying them into the `/public` directory for you, we can predictably load them in Blade views, and also ensure that third party packages are able to load their assets without having to worry about where they are located.
...
The `WellCMSAsset` facade should be used in the `boot()` method of a service provider. It can be used inside an application service provider such as `AppServiceProvider`, or inside a plugin service provider.

The `WellCMSAsset` facade has one main method, `register()`, which accepts an array of assets to register:
...
WellCMSAsset::register([
    // ...
], package: 'danharrin/wellcms-blog');
```
...
To register a CSS file with the asset system, use the `WellCMSAsset::register()` method in the `boot()` method of a service provider. You must pass in an array of `Css` objects, which each represents a CSS file that should be registered in the asset system.

Each `Css` object has a unique ID and a path to the CSS file:

```php
use WellCMS\Support\Assets\Css;
use WellCMS\Support\Facades\WellCMSAsset;
...
Now, when the `php artisan wellcms:assets` command is run, this CSS file is copied into the `/public` directory. In addition, it is now loaded into all Blade views that use WellCMS. If you're interested in only loading the CSS when it is required by an element on the page, check out the [Lazy loading CSS](#lazy-loading-css) section.
...
export default {
    content: [
        ...
        './vendor/danharrin/wellcms-blog/resources/views/**/*.blade.php', // Your plugin's vendor directory
    ],
    // ...
}
```

This means that when they build their Tailwind CSS file, it will include all the utility classes that are used in your plugin's views, as well as the utility classes that are used in their application and the WellCMS core.
...
By default, all CSS files registered with the asset system are loaded in the `<head>` of every WellCMS page. This is the simplest way to load CSS files, but sometimes they may be quite heavy and not required on every page. In this case, you can leverage the [Alpine.js Lazy Load Assets](https://github.com/tanthammar/alpine-lazy-load-assets) package that comes bundled with WellCMS. It allows you to easily load CSS files on-demand using Alpine.js. The premise is very simple, you use the `x-load-css` directive on an element, and when that element is loaded onto the page, the specified CSS files are loaded into the `<head>` of the page. This is perfect for both small UI elements and entire pages that require a CSS file:

```blade
<div
    x-data="{}"
    x-load-css="[@js(\WellCMS\Support\Facades\WellCMSAsset::getStyleHref('custom-stylesheet'))]"
>
    <!-- ... -->
</div>
```
...
If your CSS file was [registered to a plugin](#registering-assets-for-a-plugin), you must pass that in as the second argument to the `WellCMSAsset::getStyleHref()` method:

```blade
<div
    x-data="{}"
    x-load-css="[@js(\WellCMS\Support\Facades\WellCMSAsset::getStyleHref('custom-stylesheet', package: 'danharrin/wellcms-blog'))]"
>
    <!-- ... -->
</div>
```
...
Sometimes, you may wish to use dynamic data from the backend in CSS files. To do this, you can use the `WellCMSAsset::registerCssVariables()` method in the `boot()` method of a service provider:

```php
use WellCMS\Support\Facades\WellCMSAsset;

WellCMSAsset::registerCssVariables([
...
To register a JavaScript file with the asset system, use the `WellCMSAsset::register()` method in the `boot()` method of a service provider. You must pass in an array of `Js` objects, which each represents a JavaScript file that should be registered in the asset system.
...
Now, when the `php artisan wellcms:assets` command is run, this JavaScript file is copied into the `/public` directory. In addition, it is now loaded into all Blade views that use WellCMS. If you're interested in only loading the JavaScript when it is required by an element on the page, check out the [Lazy loading JavaScript](#lazy-loading-javascript) section.

### Lazy loading JavaScript

By default, all JavaScript files registered with the asset system are loaded at the bottom of every WellCMS page. This is the simplest way to load JavaScript files, but sometimes they may be quite heavy and not required on every page. In this case, you can leverage the [Alpine.js Lazy Load Assets](https://github.com/tanthammar/alpine-lazy-load-assets) package that comes bundled with WellCMS. It allows you to easily load JavaScript files on-demand using Alpine.js. The premise is very simple, you use the `x-load-js` directive on an element, and when that element is loaded onto the page, the specified JavaScript files are loaded at the bottom of the page. This is perfect for both small UI elements and entire pages that require a JavaScript file:

```blade
<div
    x-data="{}"
    x-load-js="[@js(\WellCMS\Support\Facades\WellCMSAsset::getScriptSrc('custom-script'))]"
>
    <!-- ... -->
</div>
```
...
If your JavaScript file was [registered to a plugin](#registering-assets-for-a-plugin), you must pass that in as the second argument to the `WellCMSAsset::getScriptSrc()` method:

```blade
<div
    x-data="{}"
    x-load-js="[@js(\WellCMS\Support\Facades\WellCMSAsset::getScriptSrc('custom-script', package: 'danharrin/wellcms-blog'))]"
>
    <!-- ... -->
</div>
```
...
<LaracastsBanner
    ...
    description="Watch the Build Advanced Components for WellCMS series on Laracasts - it will teach you how to get started with Async Alpine components into a plugin."
    url="https://laracasts.com/series/build-advanced-components-for-wellcms/episodes/15"
    series="building-advanced-components"
/>
...
Now, you need to tell WellCMS to publish this compiled JavaScript file into the `/public` directory of the Laravel application, so it is accessible to the browser. To do this, you can use the `WellCMSAsset::register()` method in the `boot()` method of a service provider, passing in an `AlpineComponent` object:
...
When you run `php artisan wellcms:assets`, the compiled file will be copied into the `/public` directory.

Finally, you can load this asynchronous Alpine component in your view using `x-load` attributes and the `WellCMSAsset::getAlpineComponentSrc()` method:

```blade
<div
    x-load
    x-load-src="{{ \WellCMS\Support\Facades\WellCMSAsset::getAlpineComponentSrc('test-component') }}"
...
Sometimes, you may wish to make data from the backend available to JavaScript files. To do this, you can use the `WellCMSAsset::registerScriptData()` method in the `boot()` method of a service provider:

```php
use WellCMS\Support\Facades\WellCMSAsset;

WellCMSAsset::registerScriptData([
    'user' => [
        'name' => auth()->user()?->name,
    ],
]);
...
Now, you can access that data from any JavaScript file at runtime, using the `window.wellcmsData` object:

```js
window.wellcmsData.user.name // 'Dan Harrin'
```

### Registering JavaScript files from a URL

If you want to register a JavaScript file from a URL, you may do so. These assets will be loaded on every page as normal, but not copied into the `/public` directory when the `php artisan wellcms:assets` command is run. This is useful for registering external scripts from a CDN, or scripts that you are already compiling directly into the `/public` directory:
