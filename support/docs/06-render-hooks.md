...
WellCMS allows you to render Blade content at various points in the frameworks views. It's useful for plugins to be able to inject HTML into the framework. Also, since WellCMS does not recommend publishing the views due to an increased risk of breaking changes, it's also useful for users.

## Registering render hooks

To register render hooks, you can call `WellCMSView::registerRenderHook()` from a service provider or middleware. The first argument is the name of the render hook, and the second argument is a callback that returns the content to be rendered:
...
WellCMSView::registerRenderHook(
...
```php
    use WellCMS\Tables\View\TablesRenderHook;
```
...
```php
    use WellCMS\Widgets\View\WidgetsRenderHook;
```
...
WellCMSView::registerRenderHook(
    ...
    scopes: \App\WellCMS\Resources\UserResource\Pages\EditUser::class,
);
```
...
WellCMSView::registerRenderHook(
    ...
    fn (): View => view('warning-banner'),
    scopes: [
        \App\WellCMS\Resources\UserResource\Pages\CreateUser::class,
        \App\WellCMS\Resources\UserResource\Pages\EditUser::class,
    ],
);
```
...
WellCMSView::registerRenderHook(
    PanelsRenderHook::PAGE_START,
    fn (): View => view('warning-banner'),
    scopes: \App\WellCMS\Resources\UserResource::class,
);
```
...
```blade
{{ \WellCMS\Support\Facades\WellCMSView::renderHook(\WellCMS\View\PanelsRenderHook::PAGE_START) }}
```
...
```blade
{{ \WellCMS\Support\Facades\WellCMSView::renderHook(\WellCMS\View\PanelsRenderHook::PAGE_START, scopes: [static::class, \App\WellCMS\Resources\UserResource::class]) }}
```
