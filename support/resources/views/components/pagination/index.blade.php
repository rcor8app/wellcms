...
<nav
    aria-label="{{ __('wellcms::components/pagination.label') }}"
    role="navigation"
    {{
        $attributes->class([
...
>
    ...
    @if (! $isSimple)
        ...
        >
            {{
                trans_choice(
                    'wellcms::components/pagination.overview',
                    $paginator->total(),
                    [
    ...
    @if (count($pageOptions) > 1)
        <div class="col-start-2 justify-self-center">
            <label class="fi-pagination-records-per-page-select fi-compact">
                <x-wellcms::input.wrapper>
                    ...
                    >
                        @foreach ($pageOptions as $option)
                            <option value="{{ $option }}">
                                {{ $option === 'all' ? __('wellcms::components/pagination.fields.records_per_page.options.all') : $option }}
                            </option>
                        @endforeach
                    </x-wellcms::input.select>
                </x-wellcms::input.wrapper>
                ...
                <span class="sr-only">
                    {{ __('wellcms::components/pagination.fields.records_per_page.label') }}
                </span>
            </label>
    ...
    @if ((! $isSimple) && $paginator->hasPages())
        ...
        >
            @if (! $paginator->onFirstPage())
                @if ($extremeLinks)
                    <x-wellcms::pagination.item
                        :aria-label="__('wellcms::components/pagination.actions.first.label')"
                ...
                <x-wellcms::pagination.item
                    :aria-label="__('wellcms::components/pagination.actions.previous.label')"
            ...
            @foreach ($paginator->render()->offsetGet('elements') as $element)
                @if (is_string($element))
                    <x-wellcms::pagination.item disabled :label="$element" />
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-wellcms::pagination.item
                            ...
                            :aria-label="trans_choice('wellcms::components/pagination.actions.go_to_page.label', $page, ['page' => $page])"
                            :label="$page"
            ...
            @if ($paginator->hasMorePages())
                <x-wellcms::pagination.item
                    :aria-label="__('wellcms::components/pagination.actions.next.label')"
                ...
                @if ($extremeLinks)
                    <x-wellcms::pagination.item
                        :aria-label="__('wellcms::components/pagination.actions.last.label')"
