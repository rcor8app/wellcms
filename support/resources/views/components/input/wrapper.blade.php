...
@php
    $prefixActions = array_filter(
        $prefixActions,
        fn (\WellCMS\Forms\Components\Actions\Action $prefixAction): bool => $prefixAction->isVisible(),
    );

    $suffixActions = array_filter(
        $suffixActions,
        fn (\WellCMS\Forms\Components\Actions\Action $suffixAction): bool => $suffixAction->isVisible(),
    );
    ...
    $disabledWrapperClasses = 're-disabled bg-gray-50 dark:bg-transparent';
    $validWrapperClasses = 'ring-gray-950/10';
    $invalidWrapperClasses = 're-invalid ring-danger-600 dark:ring-danger-500';
    ...
    $labelClasses = 're-input-wrp-label whitespace-nowrap text-sm text-gray-500 dark:text-gray-400';

    $getIconClasses = fn (string | array $color = 'gray'): string => \Illuminate\Support\Arr::toCssClasses([
        're-input-wrp-icon h-5 w-5',
        match ($color) {
...
<div
    ...
    {{
        $attributes
            ...
            ->class([
                're-input-wrp flex rounded-lg shadow-sm ring-1 transition duration-75',
...
>
    @if ($hasPrefix || $hasLoadingIndicator)
        <div
            @if (! $hasPrefix)
                wire:loading.delay.{{ config('wellcms.livewire_loading_delay', 'default') }}.flex
                wire:target="{{ $loadingIndicatorTarget }}"
                wire:key="{{ \Illuminate\Support\Str::random() }}" {{-- Makes sure the loading indicator gets hidden again. --}}
            @endif
            @class([
                're-input-wrp-prefix items-center gap-x-3 ps-3',
                'flex' => $hasPrefix,
                'hidden' => ! $hasPrefix,
                    ...
                    :attributes="
                        \WellCMS\Support\prepare_inherited_attributes(
                            new \Illuminate\View\ComponentAttributeBag([
                                ...
                                'icon' => $prefixIcon,
                                'wire:loading.remove.delay.' . config('wellcms.livewire_loading_delay', 'default') => $hasLoadingIndicator,
            ...
            @if ($hasLoadingIndicator)
                <x-wellcms::loading-indicator
                    :attributes="
                        \WellCMS\Support\prepare_inherited_attributes(
                            new \Illuminate\View\ComponentAttributeBag([
                                'wire:loading.delay.' . config('wellcms.livewire_loading_delay', 'default') => $hasPrefix,
    ...
    <div
        @if ($hasLoadingIndicator && (! $hasPrefix))
            @if ($inlinePrefix)
                wire:loading.delay.{{ config('wellcms.livewire_loading_delay', 'default') }}.class.remove="ps-3"
            @endif

            wire:target="{{ $loadingIndicatorTarget }}"
        @endif
        @class([
            're-input-wrp-input min-w-0 flex-1',
    ...
    @if ($hasSuffix)
        <div
            @class([
                're-input-wrp-suffix flex items-center gap-x-3 pe-3',
