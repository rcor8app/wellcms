...
trait CanGeneratePanels
{
    ...
    public function generatePanel(?string $id = null, string $default = '', string $placeholder = '', bool $force = false): bool
    {
        ...
        if ($isLaravel11OrHigherWithBootstrapProvidersFile) {
            ...
            if (! Str::contains($appConfig, "App\\Providers\\WellCMS\\{$class}::class")) {
                file_put_contents(config_path('app.php'), str_replace(
                    'App\\Providers\\RouteServiceProvider::class,',
                    "App\\Providers\\WellCMS\\{$class}::class," . PHP_EOL . '        App\\Providers\\RouteServiceProvider::class,',
                    $appConfig,
                ));
            }
        }

        $this->components->info("WellCMS panel [{$path}] created successfully.");
