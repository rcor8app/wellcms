...
#[AsCommand(name: 'wellcms:check-translations')]
class CheckTranslationsCommand extends Command implements PromptsForMissingInput
{
    protected $signature = 'wellcms:check-translations
    ...
    protected function scan(string $package): void
    {
        $localeRootDirectory = match ($source = $this->option('source')) {
            ...
            'vendor' => base_path("vendor/wellcms/{$package}/resources/lang"),
        ...
        collect($filesystem->directories($localeRootDirectory))
            ...
            ->each(function (string $locale, string $localeDir) use ($filesystem, $localeRootDirectory, $package) {
                ...
                if ($missingFiles->count() > 0 && $removedFiles->count() > 0) {
                    warning("[!] Package wellcms/{$package} has {$missingFiles->count()} missing translation " . Str::plural('file', $missingFiles->count()) . " and {$removedFiles->count()} removed translation " . Str::plural('file', $missingFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                } elseif ($missingFiles->count() > 0) {
                    warning("[!] Package wellcms/{$package} has {$missingFiles->count()} missing translation " . Str::plural('file', $missingFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                } elseif ($removedFiles->count() > 0) {
                    warning("[!] Package wellcms/{$package} has {$removedFiles->count()} removed translation " . Str::plural('file', $removedFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                }
                ...
                collect($files)
                    ...
                    ->tap(function (Collection $files) use ($locale, $package) {
                        ...
                        if ($missingKeysCount == 0 && $removedKeysCount == 0) {
                            info("[✓] Package wellcms/{$package} has no missing or removed translation keys for {$locale}!\n");
                        } elseif ($missingKeysCount > 0 && $removedKeysCount > 0) {
                            warning("[!] Package wellcms/{$package} has {$missingKeysCount} missing translation " . Str::plural('key', $missingKeysCount) . " and {$removedKeysCount} removed translation " . Str::plural('key', $removedKeysCount) . " for {$locale}.\n");
                        } elseif ($missingKeysCount > 0) {
                            warning("[!] Package wellcms/{$package} has {$missingKeysCount} missing translation " . Str::plural('key', $missingKeysCount) . " for {$locale}.\n");
                        } else {
                            warning("[!] Package wellcms/{$package} has {$removedKeysCount} removed translation " . Str::plural('key', $removedKeysCount) . " for {$locale}.\n");
                        }
                    })
