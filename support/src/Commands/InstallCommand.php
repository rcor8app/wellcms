...
#[AsCommand(name: 'wellcms:install')]
class InstallCommand extends Command
{
    use CanGeneratePanels;
    use CanManipulateFiles;

    protected $signature = 'wellcms:install {--scaffold} {--actions} {--forms} {--infolists} {--notifications} {--panels} {--tables} {--widgets} {--F|force}';

    protected $description = 'Install WellCMS';
    ...
    protected function askToStar(): void
    {
        ...
        if (confirm(
            label: 'All done! Would you like to show some love by starring the WellCMS repo on GitHub?',
            default: true,
        )) {
            if (PHP_OS_FAMILY === 'Darwin') {
                exec('open https://github.com/wellcmsphp/wellcms');
            }
            if (PHP_OS_FAMILY === 'Linux') {
                exec('xdg-open https://github.com/wellcmsphp/wellcms');
            }
            if (PHP_OS_FAMILY === 'Windows') {
                exec('start https://github.com/wellcmsphp/wellcms');
            }
    ...
    protected function installAdminPanel(): bool
    {
        if (! class_exists(PanelProvider::class)) {
            $this->components->error('Please require [wellcms/wellcms] before attempting to install the Panel Builder.');
    ...
    protected function installScaffolding(): void
    {
        ...
        // Install wellcms/notifications into the layout Blade file
        if (
            $this->option('actions') ||
    ...
    protected function installUpgradeCommand(): void
    {
        ...
        $command = '@php artisan wellcms:upgrade';
