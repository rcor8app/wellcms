...
trait HasRoutes
{
    ...
    public function generateRouteName(string $name): string
    {
        ...
        if (count($this->domains) > 1) {
            $domain = WellCMS::getCurrentDomain(testingDomain: Arr::first($this->domains)) . '.';
        }

        return "wellcms.{$this->getId()}.{$domain}{$name}";
    }
    ...
    public function getUrl(?Model $tenant = null): ?string
    {
        ...
        if ((! $tenant) && $hasTenancy && $this->auth()->hasUser()) {
            $tenant = WellCMS::getUserDefaultTenant($this->auth()->user());
        }

        if ((! $tenant) && $hasTenancy) {
            return ($this->hasTenantRegistration() && wellcms()->getTenantRegistrationPage()::canView()) ?
                $this->getTenantRegistrationUrl() :
                null;
        }

        if ($tenant) {
            $originalTenant = WellCMS::getTenant();
            WellCMS::setTenant($tenant, isQuiet: true);
        }
        ...
        try {
            ...
            if ($tenant) {
                WellCMS::setTenant($originalTenant, isQuiet: true);
            }
