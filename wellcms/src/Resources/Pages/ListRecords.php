...
use WellCMS\Resources\Concerns\HasTabs;
use WellCMS\Tables;
...
class ListRecords extends Page implements Tables\Contracts\HasTable
{
     ...
     * @var view-string
     */
    protected static string $view = 'wellcms-panels::resources.pages.list-records';
    ...
    {
        return static::$breadcrumb ?? __('wellcms-panels::resources/pages/list-records.breadcrumb');
    }
    ...
    protected function configureCreateAction(CreateAction | Tables\Actions\CreateAction $action): void
    {
        ...
        if (($action instanceof CreateAction) && static::getResource()::isScopedToTenant()) {
            $action->relationship(($tenant = WellCMS::getTenant()) ? fn (): Relation => static::getResource()::getTenantRelationship($tenant) : null);
        }
