...
use WellCMS\Events\TenantSet;
use WellCMS\Exceptions\NoDefaultPanelSetException;
use WellCMS\GlobalSearch\Contracts\GlobalSearchProvider;
...
use WellCMS\Navigation\NavigationGroup;
use WellCMS\Navigation\NavigationItem;
...
class WellCMSManager
{
    ...
    public function getTenantAvatarUrl(Model $tenant): string
    {
        ...
        if ($tenant instanceof HasAvatar) {
            $avatar = $tenant->getWellCMSAvatarUrl();
        }
    ...
    public function getUserAvatarUrl(Model | Authenticatable $user): string
    {
        if ($user instanceof HasAvatar) {
            $avatar = $user->getWellCMSAvatarUrl();
        } else {
    ...
    /**
     * @deprecated Use the `\WellCMS\Support\Facades\WellCMSView::renderHook()` method instead.
     */
    public function renderHook(string $name): Htmlable
    {
        return WellCMSView::renderHook($name);
    }

    public function serving(Closure $callback): void
    {
        Event::listen(ServingWellCMS::class, $callback);
    }
    ...
    public function registerNavigationGroups(array $groups): void
    {
        try {
            ...
            throw new Exception('Please use the `navigationGroups()` method on the panel configuration to register navigation groups. See the documentation - https://wellcmsphp.com/docs/panels/navigation#customizing-navigation-groups');
        }
    }
    ...
    public function registerNavigationItems(array $items): void
    {
        try {
            ...
            throw new Exception('Please use the `navigationItems()` method on the panel configuration to register navigation items. See the documentation - https://wellcmsphp.com/docs/panels/navigation#registering-custom-navigation-items');
        }
    }
    ...
    {
        WellCMSView::registerRenderHook($name, $hook);
    }
    ...
    /**
     * @deprecated Register scripts using the `WellCMSAsset` facade instead.
     *
     * @param  array<mixed>  $scripts
     */
    public function registerScripts(array $scripts, bool $shouldBeLoadedBeforeCoreScripts = false): void
    {
        throw new Exception('Please use the `WellCMSAsset` facade to register scripts. See the documentation - https://wellcmsphp.com/docs/support/assets#registering-javascript-files');
    }

    /**
     * @deprecated Register script data using the `WellCMSAsset` facade instead.
     *
     * @param  array<string, mixed>  $data
     */
    public function registerScriptData(array $data): void
    {
        WellCMSAsset::registerScriptData($data);
    }

    /**
     * @deprecated Register styles using the `WellCMSAsset` facade instead.
     *
     * @param  array<mixed>  $styles
     */
    public function registerStyles(array $styles): void
    {
        throw new Exception('Please use the `WellCMSAsset` facade to register styles. See the documentation - https://wellcmsphp.com/docs/support/assets#registering-css-files');
    }
    ...
    public function registerUserMenuItems(array $items): void
    {
        try {
            ...
            throw new Exception('Please use the `userMenuItems()` method on the panel configuration to register user menu items. See the documentation - https://wellcmsphp.com/docs/panels/navigation#customizing-the-user-menu');
        }
    }
    ...
    public function getCurrentDomain(?string $testingDomain = null): string
    {
        ...
        if (app()->runningInConsole()) {
            throw new Exception('The current domain is not set, but multiple domains are registered for the panel. Please use [WellCMS::currentDomain(\'example.com\')] to set the current domain to ensure that panel URLs are generated correctly.');
        }
