...
use WellCMS\Clusters\Cluster;
use WellCMS\Facades\WellCMS;
use WellCMS\Forms\Commands\Concerns\CanGenerateForms;
use WellCMS\Panel;
use WellCMS\Support\Commands\Concerns\CanIndentStrings;
use WellCMS\Support\Commands\Concerns\CanManipulateFiles;
use WellCMS\Support\Commands\Concerns\CanReadModelSchemas;
use WellCMS\Tables\Commands\Concerns\CanGenerateTables;
...
#[AsCommand(name: 'make:wellcms-resource')]
class MakeResourceCommand extends Command
{
    ...
    protected $description = 'Create a new WellCMS resource class and default page classes';

    protected $signature = 'make:wellcms-resource {name?} {--model-namespace=} {--soft-deletes} {--view} {--G|generate} {--S|simple} {--panel=} {--model} {--migration} {--factory} {--F|force}';
    ...
    public function handle(): int
    {
        ...
        $namespace = (count($resourceNamespaces) > 1) ?
            ...
            (Arr::first($resourceNamespaces) ?? 'App\\WellCMS\\Resources');
        $path = (count($resourceDirectories) > 1) ?
            $resourceDirectories[array_search($namespace, $resourceNamespaces)] :
            (Arr::first($resourceDirectories) ?? app_path('WellCMS/Resources/'));
        ...
        if ($this->option('simple')) {
            $this->copyStubToApp('ResourceManagePage', $manageResourcePagePath, [
                'baseResourcePage' => 'WellCMS\\Resources\\Pages\\ManageRecords' . ($needsAlias ? ' as BaseManageRecords' : ''),
            ...
            $this->copyStubToApp('ResourceListPage', $listResourcePagePath, [
                'baseResourcePage' => 'WellCMS\\Resources\\Pages\\ListRecords' . ($needsAlias ? ' as BaseListRecords' : ''),
            ...
            $this->copyStubToApp('ResourcePage', $createResourcePagePath, [
                'baseResourcePage' => 'WellCMS\\Resources\\Pages\\CreateRecord' . ($needsAlias ? ' as BaseCreateRecord' : ''),
            ...
            if ($this->option('view')) {
                $this->copyStubToApp('ResourceViewPage', $viewResourcePagePath, [
                    'baseResourcePage' => 'WellCMS\\Resources\\Pages\\ViewRecord' . ($needsAlias ? ' as BaseViewRecord' : ''),
            ...
            $this->copyStubToApp('ResourceEditPage', $editResourcePagePath, [
                'baseResourcePage' => 'WellCMS\\Resources\\Pages\\EditRecord' . ($needsAlias ? ' as BaseEditRecord' : ''),
        ...
        $this->components->info("WellCMS resource [{$resourcePath}] created successfully.");
