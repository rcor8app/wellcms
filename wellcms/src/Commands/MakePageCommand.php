...
#[AsCommand(name: 'make:wellcms-page')]
class MakePageCommand extends Command
{
    use CanIndentStrings;
    use CanManipulateFiles;

    protected $description = 'Create a new WellCMS page class and view';

    protected $signature = 'make:wellcms-page {name?} {--R|resource=} {--T|type=} {--panel=} {--F|force}';
    ...
    public function handle(): int
    {
        ...
        if (empty($resource)) {
            ...
            $namespace = (count($pageNamespaces) > 1) ?
                ...
                (Arr::first($pageNamespaces) ?? 'App\\WellCMS\\Pages');
            $path = (count($pageDirectories) > 1) ?
                $pageDirectories[array_search($namespace, $pageNamespaces)] :
                (Arr::first($pageDirectories) ?? app_path('WellCMS/Pages/'));
        } else {
            $resourceDirectories = $panel->getResourceDirectories();
            $resourceNamespaces = $panel->getResourceNamespaces();
        ...
        if (empty($resource)) {
            ...
            $this->copyStubToApp('ResourceManageRelatedRecordsPage', $path, [
                'baseResourcePage' => "WellCMS\\Resources\\Pages\\{$resourcePage}",
            ...
            $this->copyStubToApp($resourcePage === 'custom' ? 'CustomResourcePage' : 'ResourcePage', $path, [
                'baseResourcePage' => 'WellCMS\\Resources\\Pages\\' . ($resourcePage === 'custom' ? 'Page' : $resourcePage),
        ...
        $this->components->info("WellCMS page [{$path}] created successfully.");
