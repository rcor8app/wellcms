...
#[AsCommand(name: 'make:wellcms-theme')]
class MakeThemeCommand extends Command
{
    use CanManipulateFiles;

    protected $description = 'Create a new WellCMS panel theme';

    protected $signature = 'make:wellcms-theme {panel?} {--pm=} {--F|force}';
    ...
    public function handle(): int
    {
        ...
        $cssFilePath = resource_path("css/wellcms/{$panelId}/theme.css");
        $tailwindConfigFilePath = resource_path("css/wellcms/{$panelId}/tailwind.config.js");
        ...
        $this->components->info("WellCMS theme [resources/css/wellcms/{$panelId}/theme.css] and [resources/css/wellcms/{$panelId}/tailwind.config.js] created successfully.");
        ...
        if (! $shouldInstallTailwindV3) {
            ...
            $this->components->bulletList([
                ...
                "npx tailwindcss@3 --input ./resources/css/wellcms/{$panelId}/theme.css --output ./public/css/wellcms/{$panelId}/theme.css --config ./resources/css/wellcms/{$panelId}/tailwind.config.js --minify",
                "Make sure to register the theme in the {$panelId} panel provider using `->theme(asset('css/wellcms/{$panelId}/theme.css'))`",
            ]);
        ...
        if (empty(glob(base_path('vite.config.*s')))) {
            $this->components->warn('Action is required to complete the theme setup:');
            $this->components->bulletList([
                "It looks like you don't have Vite installed. Please use your asset bundling system of choice to compile `resources/css/wellcms/{$panelId}/theme.css` into `public/css/wellcms/{$panelId}/theme.css`.",
        ...
        if (! file_exists($postcssConfigPath)) {
            ...
            $this->components->info('WellCMS theme [postcss.config.js] created successfully.');
        }

        $this->components->warn('Action is required to complete the theme setup:');
        $this->components->bulletList([
            "First, add a new item to the `input` array of `vite.config.js`: `resources/css/wellcms/{$panelId}/theme.css`.",
            "Next, register the theme in the {$panelId} panel provider using `->viteTheme('resources/css/wellcms/{$panelId}/theme.css')`",
