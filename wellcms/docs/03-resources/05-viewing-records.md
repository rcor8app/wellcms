...
```bash
php artisan make:wellcms-resource User --view
```
...
<LaracastsBanner
    title="Infolists"
    description="Watch the Rapid Laravel Development with WellCMS series on Laracasts - it will teach you the basics of adding infolists to WellCMS resources."
    url="https://laracasts.com/series/rapid-laravel-development-with-wellcms/episodes/12"
...
Check out the Infolists docs for a [guide](../../infolists/getting-started) on how to build infolists with WellCMS.
...
```bash
php artisan make:wellcms-page ViewUser --resource=UserResource --type=ViewRecord
```
...
```php
use WellCMS\Resources\Pages\ViewRecord;
...
For authorization, WellCMS will observe any [model policies](https://laravel.com/docs/authorization#creating-policies) that are registered in your app.
...
To create a View page, you should use the `make:wellcms-page` command:

```bash
php artisan make:wellcms-page ViewCustomerContact --resource=CustomerResource --type=ViewRecord
```
...
```php
protected static string $view = 'wellcms.resources.users.pages.view-user';
```

This assumes that you have created a view at `resources/views/wellcms/resources/users/pages/view-user.blade.php`.
...
To see everything that the default view contains, you can check the `vendor/wellcms/wellcms/resources/views/resources/pages/view-record.blade.php` file in your project.
