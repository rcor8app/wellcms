...
```php
use WellCMS\Forms\Components\MarkdownEditor;
use WellCMS\Forms\Components\TextInput;
use WellCMS\Forms\Components\Toggle;
use WellCMS\Forms\Components\Wizard\Step;
...
WellCMS includes an `ImportAction` that you can add to the `getHeaderActions()` of the [List page](listing-records). It allows users to upload a CSV of data to import into the resource:
...
The "importer" class [needs to be created](../../actions/prebuilt-actions/import#creating-an-importer) to tell WellCMS how to import each row of the CSV. You can learn everything about the `ImportAction` in the [Actions documentation](../../actions/prebuilt-actions/import).
...
```php
use App\WellCMS\Imports\UserImporter;
use WellCMS\Actions;
use WellCMS\Resources\Pages\CreateRecord;
...
```php
protected static string $view = 'wellcms.resources.users.pages.create-user';
```

This assumes that you have created a view at `resources/views/wellcms/resources/users/pages/create-user.blade.php`.

Here's a basic example of what that view might contain:

```blade
<x-wellcms-panels::page>
    <x-wellcms-panels::form wire:submit="create">
        {{ $this->form }}
...
To see everything that the default view contains, you can check the `vendor/wellcms/wellcms/resources/views/resources/pages/create-record.blade.php` file in your project.
