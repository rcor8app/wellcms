...
<LaracastsBanner
    ...
    description="Watch the Rapid Laravel Development with WellCMS series on Laracasts - it will teach you how to get started with the resources."
...
```bash
php artisan make:wellcms-resource Customer
```
...
```bash
php artisan make:wellcms-resource Customer --simple
```
...
If you'd like to save time, WellCMS can automatically generate the [form](#resource-forms) and [table](#resource-tables) for you, based on your model's database columns, using `--generate`:

```bash
php artisan make:wellcms-resource Customer --generate
```
...
```bash
php artisan make:wellcms-resource Customer --soft-deletes
```
...
```bash
php artisan make:wellcms-resource Customer --view
```

### Specifiying a custom model namespace

By default, WellCMS will assume that your model exists in the `App\Models` directory. You can pass a different namespace for the model using the `--model-namespace` flag:

```bash
php artisan make:wellcms-resource Customer --model-namespace=Custom\\Path\\Models
```
...
Now when [generating the resource](#automatically-generating-forms-and-tables), WellCMS will be able to locate the model and read the database schema.

### Generating the model, migration and factory at the same name

If you'd like to save time when scaffolding your resources, WellCMS can also generate the model, migration and factory for the new resource at the same time using the `--model`, `--migration` and `--factory` flags in any combination:

```bash
php artisan make:wellcms-resource Customer --model --migration --factory
```
...
<LaracastsBanner
    ...
    description="Watch the Rapid Laravel Development with WellCMS series on Laracasts - it will teach you the basics of adding a form to your resource."
    url="https://laracasts.com/series/rapid-laravel-development-with-wellcms/episodes/3"
    series="rapid-laravel-development"
/>
...
Check out the Forms docs for a [guide](../../forms/getting-started) on how to build forms with WellCMS.
...
<LaracastsBanner
    title="Table Columns"
    description="Watch the Rapid Laravel Development with WellCMS series on Laracasts - it will teach you the basics of adding a table to your resource."
    url="https://laracasts.com/series/rapid-laravel-development-with-wellcms/episodes/9"
    series="rapid-laravel-development"
/>
...
For authorization, WellCMS will observe any [model policies](https://laravel.com/docs/authorization#creating-policies) that are registered in your app. The following methods are used:
...
- `delete()` is used to prevent a single record from being deleted. `deleteAny()` is used to prevent records from being bulk deleted. WellCMS uses the `deleteAny()` method because iterating through multiple records and checking the `delete()` policy is not very performant.
- `forceDelete()` is used to prevent a single soft-deleted record from being force-deleted. `forceDeleteAny()` is used to prevent records from being bulk force-deleted. WellCMS uses the `forceDeleteAny()` method because iterating through multiple records and checking the `forceDelete()` policy is not very performant.
- `restore()` is used to prevent a single soft-deleted record from being restored. `restoreAny()` is used to prevent records from being bulk restored. WellCMS uses the `restoreAny()` method because iterating through multiple records and checking the `restore()` policy is not very performant.
...
{
    return __('wellcms/resources/customer.label');
}
```
...
{
    return __('wellcms/resources/customer.plural_label');
}
```

### Automatic model label capitalization

By default, WellCMS will automatically capitalize each word in the model label, for some parts of the UI. For example, in page titles, the navigation menu, and the breadcrumbs.
...
WellCMS will automatically generate a navigation menu item for your resource using the [plural label](#plural-label).
...
{
    return __('wellcms/resources/customer.navigation_label');
}
```
...
{
    return __('wellcms/navigation.groups.shop');
}
```
...
{
    return __('wellcms/navigation.groups.shop.items.products');
}
```
...
WellCMS provides `getUrl()` static method on resource classes to generate URLs to resources and specific pages within them. Traditionally, you would need to construct the URL by hand or by using Laravel's `route()` helper, but these methods depend on knowledge of the resource's slug or route naming conventions.
...
```php
use App\WellCMS\Resources\CustomerResource;
use WellCMS\Tables\Actions\EditAction;
...
```php
use App\WellCMS\Resources\CustomerResource;
use WellCMS\Actions\CreateAction;
...
Within WellCMS, every query to your resource model will start with the `getEloquentQuery()` method.
...
By default, WellCMS will observe all global scopes that are registered to your model. However, this may not be ideal if you wish to access, for example, soft deleted records.

To overcome this, you may override the `getEloquentQuery()` method that WellCMS uses:
...
By default, WellCMS will generate a URL based on the name of the resource. You can customize this by setting the `$slug` property on the resource:
...
```php
use App\WellCMS\Resources\CustomerResource\Pages;
use WellCMS\Resources\Pages\Page;
