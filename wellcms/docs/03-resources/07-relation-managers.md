...
WellCMS provides many ways to manage relationships in the app. Which feature you should use depends on the type of relationship you are managing, and which UI you are looking for.
...
```php
use WellCMS\Forms\Components\Fieldset;
use WellCMS\Forms\Components\FileUpload;
...
<LaracastsBanner
    ...
    description="Watch the Rapid Laravel Development with WellCMS series on Laracasts - it will teach you the basics of adding relation managers to WellCMS resources."
    url="https://laracasts.com/series/rapid-laravel-development-with-wellcms/episodes/13"
    series="rapid-laravel-development"
/>

To create a relation manager, you can use the `make:wellcms-relation-manager` command:

```bash
php artisan make:wellcms-relation-manager CategoryResource posts title
```
...
Relation managers are usually displayed on either the Edit or View page of a resource. On the View page, WellCMS will automatically hide all actions that modify the relationship, such as create, edit, and delete. We call this "read-only mode", and it is there by default to preserve the read-only behavior of the View page. However, you can disable this behavior, by overriding the `isReadOnly()` method on the relation manager class to return `false` all the time:
...
```bash
php artisan make:wellcms-relation-manager CategoryResource posts title --soft-deletes
```
...
WellCMS is able to attach and detach records for `BelongsToMany` and `MorphToMany` relationships.
...
```bash
php artisan make:wellcms-relation-manager CategoryResource posts title --attach
```
...
```php
use WellCMS\Tables\Actions\AttachAction;
...
WellCMS is able to associate and dissociate records for `HasMany` and `MorphMany` relationships.
...
```bash
php artisan make:wellcms-relation-manager CategoryResource posts title --associate
```
...
```php
use WellCMS\Tables\Actions\AssociateAction;
...
```bash
php artisan make:wellcms-relation-manager CategoryResource posts title --view
```
...
All methods in WellCMS accept a callback which you can access `$livewire->ownerRecord` in.
...
```php
use WellCMS\Resources\RelationManagers\RelationGroup;
...
```php
use WellCMS\Resources\Pages\ContentTabPosition;
...
```php
use App\WellCMS\Resources\Blog\PostResource;
use WellCMS\Forms\Form;
use WellCMS\Tables\Table;
...
If you're sharing a form component from the resource with the relation manager, you may want to hide it on the relation manager. This is especially useful if you want to hide a `Select` field for the owner record in the relation manager, since WellCMS will handle this for you anyway. To do this, you may use the `hiddenOn()` method, passing the name of the relation manager:

```php
use App\WellCMS\Resources\Blog\PostResource\RelationManagers\CommentsRelationManager;
use WellCMS\Forms\Components\Select;
...
```php
use App\WellCMS\Resources\Blog\PostResource\RelationManagers\CommentsRelationManager;
use WellCMS\Tables\Columns\TextColumn;
...
```php
use App\WellCMS\Resources\Blog\PostResource\RelationManagers\CommentsRelationManager;
use WellCMS\Tables\Filters\SelectFilter;
...
The relation manager uses the concept of a "record title attribute" to determine which attribute of the related model should be used to identify it. When creating a relation manager, this attribute is passed as the third argument to the `make:wellcms-relation-manager` command:
...
To create a relation page, you should use the `make:wellcms-page` command:

```bash
php artisan make:wellcms-page ManageCustomerAddresses --resource=CustomerResource --type=ManageRelatedRecords
```
...
> When using a relation page, you do not need to generate a relation manager with `make:wellcms-relation-manager`, and you do not need to register it in the `getRelations()` method of the resource.
