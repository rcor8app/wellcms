...
```php
use WellCMS\Resources\Pages\EditRecord;
...
To create an Edit page, you should use the `make:wellcms-page` command:

```bash
php artisan make:wellcms-page EditCustomerContact --resource=CustomerResource --type=EditRecord
```
...
```php
protected static string $view = 'wellcms.resources.users.pages.edit-user';
```

This assumes that you have created a view at `resources/views/wellcms/resources/users/pages/edit-user.blade.php`.

Here's a basic example of what that view might contain:

```blade
<x-wellcms-panels::page>
    <x-wellcms-panels::form wire:submit="save">
        {{ $this->form }}
...
To see everything that the default view contains, you can check the `vendor/wellcms/wellcms/resources/views/resources/pages/edit-record.blade.php` file in your project.
