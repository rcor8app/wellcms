...
WellCMS plugins build on top of the concepts of Laravel packages and allow you to ship and consume reusable features for any WellCMS panel. They can be added to each panel one at a time, and are also configurable differently per-panel.
...
namespace DanHarrin\WellCMSBlog;

use DanHarrin\WellCMSBlog\Pages\Settings;
use DanHarrin\WellCMSBlog\Resources\CategoryResource;
use DanHarrin\WellCMSBlog\Resources\PostResource;
...
```php
use DanHarrin\WellCMSBlog\BlogPlugin;
...
```php
use DanHarrin\WellCMSBlog\Resources\AuthorResource;
use WellCMS\Contracts\Plugin;
use WellCMS\Panel;
...
Additionally, you can use the unique ID of the plugin to access any of its configuration options from outside the plugin class. To do this, pass the ID to the `wellcms()` method:

```php
wellcms('blog')->hasAuthorResource()
```
...
class BlogPlugin implements Plugin
{
    ...
    {
        return wellcms(app(static::class)->getId());
    }
...
"extra": {
    "laravel": {
        "providers": [
            "DanHarrin\\WellCMSBlog\\BlogPanelProvider"
        ]
    }
}
```
