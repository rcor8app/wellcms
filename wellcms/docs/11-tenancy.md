...
Multi-tenancy is a very sensitive topic. It's important to understand the security implications of multi-tenancy and how to properly implement it. If implemented partially or incorrectly, data belonging to one tenant may be exposed to another tenant. WellCMS provides a set of tools to help you implement multi-tenancy in your application, but it is up to you to understand how to use them. WellCMS does not provide any guarantees about the security of your application. It is your responsibility to ensure that your application is secure. Please see the [security](#tenancy-security) section for more information.

## Simple one-to-many tenancy

The term "multi-tenancy" is broad and may mean different things in different contexts. WellCMS's tenancy system implies that the user belongs to **many** tenants (*organizations, teams, companies, etc.*) and may switch between them.

If your case is simpler and you don't need a many-to-many relationship, then you don't need to set up the tenancy in WellCMS. You could use [observers](https://laravel.com/docs/eloquent#observers) and [global scopes](https://laravel.com/docs/eloquent#global-scopes) instead.
...
You'll also need to tell WellCMS which tenants a user belongs to. You can do this by implementing the `HasTenants` interface on the `App\Models\User` model:
...
class User extends Authenticatable implements WellCMSUser, HasTenants
{
    // ...
...
In this example, users belong to many teams, so there is a `teams()` relationship. The `getTenants()` method returns the teams that the user belongs to. WellCMS uses this to list the tenants that the user has access to.
...
To set up a registration page, you'll need to create a new page class that extends `WellCMS\Pages\Tenancy\RegisterTenant`. This is a full-page Livewire component. You can put this anywhere you want, such as `app/WellCMS/Pages/Tenancy/RegisterTeam.php`:

```php
namespace App\WellCMS\Pages\Tenancy;

use App\Models\Team;
use WellCMS\Forms\Components\TextInput;
use WellCMS\Forms\Form;
use WellCMS\Pages\Tenancy\RegisterTenant;
...
Now, we need to tell WellCMS to use this page. We can do this in the [configuration](configuration):

```php
use App\WellCMS\Pages\Tenancy\RegisterTeam;
use WellCMS\Panel;
...
To set up a profile page, you'll need to create a new page class that extends `WellCMS\Pages\Tenancy\EditTenantProfile`. This is a full-page Livewire component. You can put this anywhere you want, such as `app/WellCMS/Pages/Tenancy/EditTeamProfile.php`:
...
use WellCMS\Forms\Components\TextInput;
use WellCMS\Forms\Form;
use WellCMS\Pages\Tenancy\EditTenantProfile;
...
```php
use App\WellCMS\Pages\Tenancy\EditTeamProfile;
use WellCMS\Panel;
...
Anywhere in the app, you can access the tenant model for the current request using `WellCMS::getTenant()`:
...
WellCMS provides a billing integration with [Laravel Spark](https://spark.laravel.com). Your users can start subscriptions and manage their billing information.
...
Now, you can install the WellCMS billing provider for Spark using Composer:

```bash
composer require wellcms/spark-billing-provider
```

In the [configuration](configuration), set Spark as the `tenantBillingProvider()`:

```php
use WellCMS\Billing\Providers\SparkBillingProvider;
use WellCMS\Panel;
...
Billing integrations are quite simple to write. You just need a class that implements the `WellCMS\Billing\Providers\Contracts\Provider` interface. This interface has two methods.
...
use WellCMS\Billing\Providers\Contracts\Provider;
...
```php
use App\WellCMS\Pages\Settings;
use WellCMS\Navigation\MenuItem;
use WellCMS\Panel;
...
However, this is a sign that WellCMS's tenancy feature is not suitable for your project. If each user only belongs to one tenant, you should stick to [simple one-to-many tenancy](#simple-one-to-many-tenancy).

## Setting up avatars

Out of the box, WellCMS uses [ui-avatars.com](https://ui-avatars.com) to generate avatars based on a user's name. However, if you user model has an `avatar_url` attribute, that will be used instead. To customize how WellCMS gets a user's avatar URL, you can implement the `HasAvatar` contract:
...
use WellCMS\Models\Contracts\WellCMSUser;
use WellCMS\Models\Contracts\HasAvatar;
use Illuminate\Database\Eloquent\Model;
...
The `getWellCMSAvatarUrl()` method is used to retrieve the avatar of the current user. If `null` is returned from this method, WellCMS will fall back to [ui-avatars.com](https://ui-avatars.com).
...
When creating and listing records associated with a Tenant, WellCMS needs access to two Eloquent relationships for each resource - an "ownership" relationship that is defined on the resource model class, and a relationship on the tenant model class. By default, WellCMS will attempt to guess the names of these relationships based on standard Laravel conventions. For example, if the tenant model is `App\Models\Team`, it will look for a `team()` relationship on the resource model class. And if the resource model class is `App\Models\Post`, it will look for a `posts()` relationship on the tenant model class.
...
```php
use WellCMS\Resources\Resource;
...
By default, WellCMS will use the `name` attribute of the tenant to display its name in the app. To change this, you can implement the `HasName` contract:
...
use WellCMS\Models\Contracts\HasName;
use Illuminate\Database\Eloquent\Model;
...
The `getWellCMSName()` method is used to retrieve the name of the current user.
...
use WellCMS\Models\Contracts\HasCurrentTenantLabel;
use Illuminate\Database\Eloquent\Model;
...
When signing in, WellCMS will redirect the user to the first tenant returned from the `getTenants()` method.
...
use WellCMS\Models\Contracts\WellCMSUser;
use WellCMS\Models\Contracts\HasDefaultTenant;
use WellCMS\Models\Contracts\HasTenants;
use WellCMS\Panel;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;

class User extends Model implements WellCMSUser, HasDefaultTenant, HasTenants
{
    // ...
...
Below is a list of features that WellCMS provides to help you implement multi-tenancy in your application:
...
And here are the things that WellCMS does not currently provide:
...
- Form component and filter scoping. When using the `Select`, `CheckboxList` or `Repeater` form components, the `SelectFilter`, or any other similar WellCMS component which is able to automatically fetch "options" or other data from the database (usually using a `relationship()` method), this data is not scoped. The main reason for this is that these features often don't belong to the WellCMS Panel Builder package, and have no knowledge that they are being used within that context, and that a tenant even exists. And even if they did have access to the tenant, there is nowhere for the tenant relationship configuration to live. To scope these components, you need to pass in a query function that scopes the query to the current tenant. For example, if you were using the `Select` form component to select an `author` from a relationship, you could do this:

```php
use WellCMS\Facades\WellCMS;
use WellCMS\Forms\Components\Select;
use Illuminate\Database\Eloquent\Builder;
...
Select::make('author_id')
    ->relationship(
        name: 'author',
        titleAttribute: 'name',
        modifyQueryUsing: fn (Builder $query) => $query->whereBelongsTo(WellCMS::getTenant()),
    );
```
...
class ApplyTenantScopes
{
    public function handle(Request $request, Closure $next)
    {
        Author::addGlobalScope(
            fn (Builder $query) => $query->whereBelongsTo(WellCMS::getTenant()),
        );
