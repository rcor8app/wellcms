...
3. If you haven't already, you will need to publish the WellCMS package configuration:

```bash
php artisan vendor:publish --tag=wellcms-config
```

4. Edit the configuration at `config/wellcms.php` and uncomment the `broadcasting.echo` section - ensuring the settings are correctly configured according to your broadcasting installation.
