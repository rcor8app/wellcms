...
public function panel(Panel $panel): Panel
{
    return $panel
        // ...
        ->discoverResources(in: app_path('WellCMS/Resources'), for: 'App\\WellCMS\\Resources')
        ->discoverPages(in: app_path('WellCMS/Pages'), for: 'App\\WellCMS\\Pages')
        ->discoverClusters(in: app_path('WellCMS/Clusters'), for: 'App\\WellCMS\\Clusters');
}
```

Now, you can create a cluster with the `php artisan make:wellcms-cluster` command:

```bash
php artisan make:wellcms-cluster Settings
```

This will create a new cluster class in the `app/WellCMS/Clusters` directory:

```php
<?php

namespace App\WellCMS\Clusters;
...
```php
use App\WellCMS\Clusters\Settings;
...
When a cluster exists in your panel, and you generate new resources or pages with the `make:wellcms-resource` or `make:wellcms-page` commands, you will be asked if you want to create them inside a cluster directory, according to these guidelines. If you choose to, then WellCMS will also assign the correct `$cluster` property to the resource or page class for you. If you do not, you will need to [define the `$cluster` property](#adding-resources-and-pages-to-a-cluster) yourself.
...
{
    return __('wellcms/clusters/cluster.name');
}
```
