...
By default, the configuration file is located at `app/Providers/WellCMS/AdminPanelProvider.php`. Keep reading to learn more about [panels](#introducing-panels) and how each has [its own configuration file](#creating-a-new-panel).
...
For example, you could build a panel where users can log in at `/app` and access their dashboard, and admins can log in at `/admin` and manage the app. The `/app` panel and the `/admin` panel have their own resources, since each group of users has different requirements. WellCMS allows you to do that by providing you with the ability to create multiple panels.

### The default admin panel

When you run `wellcms:install`, a new file is created in `app/Providers/WellCMS` - `AdminPanelProvider.php`. This file contains the configuration for the `/admin` panel.
...
To create a new panel, you can use the `make:wellcms-panel` command, passing in the unique name of the new panel:

```bash
php artisan make:wellcms-panel app
```

This command will create a new panel called "app". A configuration file will be created at `app/Providers/WellCMS/AppPanelProvider.php`. You can access this panel at `/app`, but you can [customize the path](#changing-the-path) if you don't want that.

Since this configuration file is also a [Laravel service provider](https://laravel.com/docs/providers), it needs to be registered in `bootstrap/providers.php` (Laravel 11 and above) or `config/app.php` (Laravel 10 and below). WellCMS will attempt to do this for you, but if you get an error while trying to access your panel then this process has probably failed.
...
By default, WellCMS will respond to requests from all domains. If you'd like to scope it to a specific domain, you can use the `domain()` method, similar to [`Route::domain()` in Laravel](https://laravel.com/docs/routing#route-group-subdomain-routing):
...
```php
use App\WellCMS\Resources\PostResource;
use WellCMS\Panel;
...
```php
use WellCMS\Resources\Pages\CreateRecord;
...
Before these [assets](../support/assets) can be used, you'll need to run `php artisan wellcms:assets`.
...
By default, Laravel Echo will automatically connect for every panel, if credentials have been set up in the [published `config/wellcms.php` configuration file](installation#publishing-configuration). To disable this automatic connection in a panel, you can use the `broadcasting(false)` method:
