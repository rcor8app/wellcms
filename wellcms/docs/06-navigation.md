...
By default, WellCMS will register navigation items for each of your [resources](resources/getting-started), [custom pages](pages), and [clusters](clusters). These classes contain static properties and methods that you can override, to configure that navigation item.
...
{
    return __('wellcms/navigation.groups.settings.items.notifications');
}
```
    ...
    return $panel
        ...
        ->navigationItems([
            NavigationItem::make('Analytics')
                ->url('https://wellcms.pirsch.io', shouldOpenInNewTab: true)
                ->icon('heroicon-o-presentation-chart-line')
                ->group('Reports')
                ->sort(3),
            NavigationItem::make('dashboard')
                ->label(fn (): string => __('wellcms-panels::pages/dashboard.title'))
                ->url(fn (): string => Dashboard::getUrl())
                ->isActiveWhen(fn () => request()->routeIs('wellcms.admin.pages.dashboard')),
            // ...
        ]);
}
```
...
By default, WellCMS will use a sidebar navigation. You may use a top navigation instead by using the [configuration](configuration):
...
The `navigation()` method can be called from the [configuration](configuration). It allows you to build a custom navigation that overrides WellCMS's automatically generated items. This API is designed to give you complete control over the navigation.
...
use App\WellCMS\Resources\UserResource;
use WellCMS\Navigation\NavigationBuilder;
use WellCMS\Navigation\NavigationItem;
use WellCMS\Pages\Dashboard;
use WellCMS\Panel;
        ...
        ->navigation(function (NavigationBuilder $builder): NavigationBuilder {
            return $builder->items([
                NavigationItem::make('Dashboard')
                    ...
                    ->isActiveWhen(fn (): bool => request()->routeIs('wellcms.admin.pages.dashboard'))
                    ->url(fn (): string => Dashboard::getUrl()),
...
```php
use App\WellCMS\Pages\HomePageSettings;
use App\WellCMS\Resources\CategoryResource;
use App\WellCMS\Resources\PageResource;
use WellCMS\Navigation\NavigationBuilder;
use WellCMS\Navigation\NavigationGroup;
use WellCMS\Panel;
