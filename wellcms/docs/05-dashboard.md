...
WellCMS allows you to build dynamic dashboards, comprised of "widgets", very easily.

The following document will explain how to use these widgets to assemble a dashboard using the panel.

## Available widgets

WellCMS ships with these widgets:
...
You may also [create your own custom widgets](#custom-widgets) which can then have a consistent design with WellCMS's prebuilt widgets.
...
Now, in your new `app/WellCMS/Pages/Dashboard.php` file, you may override the `getColumns()` method to return a number of grid columns to use:
...
```bash
php artisan make:wellcms-widget LatestOrders --table
```
...
```bash
php artisan make:wellcms-widget BlogPostsOverview
```

This command will create two files - a widget class in the `/Widgets` directory of the WellCMS directory, and a view in the `/widgets` directory of the WellCMS views directory.
...
Now, in your new `app/WellCMS/Pages/Dashboard.php` file, you may add the `HasFiltersForm` trait, and add the `filtersForm()` method to return form components:

```php
use WellCMS\Forms\Components\DatePicker;
use WellCMS\Forms\Components\Section;
use WellCMS\Forms\Form;
use WellCMS\Pages\Dashboard as BaseDashboard;
use WellCMS\Pages\Dashboard\Concerns\HasFiltersForm;
...
use WellCMS\Widgets\StatsOverviewWidget;
use WellCMS\Widgets\Concerns\InteractsWithPageFilters;
...
use WellCMS\Forms\Form;
use WellCMS\Pages\Dashboard as BaseDashboard;
use WellCMS\Pages\Dashboard\Actions\FilterAction;
use WellCMS\Pages\Dashboard\Concerns\HasFiltersAction;
...
If you want to customize the dashboard class, for example, to [change the number of widget columns](#customizing-widget-width), create a new file at `app/WellCMS/Pages/Dashboard.php`:

```php
<?php

namespace App\WellCMS\Pages;

class Dashboard extends \WellCMS\Pages\Dashboard
{
    // ...
}
```
