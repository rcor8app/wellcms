...
By default, all `App\Models\User`s can access WellCMS locally. To allow them to access WellCMS in production, you must take a few extra steps to ensure that only the correct users have access to the app.

## Authorizing access to the panel

To set up your `App\Models\User` to access WellCMS in non-local environments, you must implement the `WellCMSUser` contract:
...
class User extends Authenticatable implements WellCMSUser
{
    // ...
...
Out of the box, WellCMS uses [ui-avatars.com](https://ui-avatars.com) to generate avatars based on a user's name. However, if your user model has an `avatar_url` attribute, that will be used instead. To customize how WellCMS gets a user's avatar URL, you can implement the `HasAvatar` contract:
...
class User extends Authenticatable implements WellCMSUser, HasAvatar
{
    // ...
...
In this example, we create a new file at `app/WellCMS/AvatarProviders/BoringAvatarsProvider.php` for [boringavatars.com](https://boringavatars.com). The `get()` method accepts a user model instance and returns an avatar URL for that user:

```php
<?php

namespace App\WellCMS\AvatarProviders;

use WellCMS\AvatarProviders\Contracts;
...
class BoringAvatarsProvider implements Contracts\AvatarProvider
{
    ...
    {
        $name = str(WellCMS::getNameForDefaultAvatar($record))
            ->trim()
            ->explode(' ')
...
```php
use App\WellCMS\AvatarProviders\BoringAvatarsProvider;
use WellCMS\Panel;
...
By default, WellCMS will use the `name` attribute of the user to display their name in the app. To change this, you can implement the `HasName` contract:
...
class User extends Authenticatable implements WellCMSUser, HasName
{
    // ...
...
If you'd like to replace these pages with your own, you can pass in any WellCMS page class to these methods.

Most people will be able to make their desired customizations by extending the base page class from the WellCMS codebase, overriding methods like `form()`, and then passing the new page class in to the configuration:

```php
use App\WellCMS\Pages\Auth\EditProfile;
use WellCMS\Panel;
...
In this example, we will customize the profile page. We need to create a new PHP class at `app/WellCMS/Pages/Auth/EditProfile.php`:

```php
<?php

namespace App\WellCMS\Pages\Auth;

use WellCMS\Forms\Components\TextInput;
use WellCMS\Forms\Form;
use WellCMS\Pages\Auth\EditProfile as BaseEditProfile;
...
This class extends the base profile page class from the WellCMS codebase. Other page classes you could extend include:

- `WellCMS\Pages\Auth\Login`
- `WellCMS\Pages\Auth\Register`
- `WellCMS\Pages\Auth\EmailVerification\EmailVerificationPrompt`
- `WellCMS\Pages\Auth\PasswordReset\RequestPasswordReset`
- `WellCMS\Pages\Auth\PasswordReset\ResetPassword`

In the `form()` method of the example, we call methods like `getNameFormComponent()` to get the default form components for the page. You can customize these components as required. For all the available customization options, see the base `EditProfile` page class in the WellCMS codebase - it contains all the methods that you can override to make changes.
...
To set the authentication guard that WellCMS uses, you can pass in the guard name to the `authGuard()` [configuration](configuration) method:
...
To set the password broker that WellCMS uses, you can pass in the broker name to the `authPasswordBroker()` [configuration](configuration) method:
...
By default, WellCMS expects to work with authenticated users only. To allow guests to access a panel, you need to avoid using components which expect a signed-in user (such as profiles, avatars), and remove the built-in Authentication middleware:
