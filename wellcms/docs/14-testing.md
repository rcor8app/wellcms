...
If you have multiple panels and you would like to test a non-default panel, you will need to tell WellCMS which panel you are testing. This can be done in the `setUp()` method of the test case, or you can do it at the start of a particular test. WellCMS usually does this in a middleware when you access the panel through a request, so if you're not making a request in your test like when testing a Livewire component, you need to set the current panel manually:

```php
use WellCMS\Facades\WellCMS;

WellCMS::setCurrentPanel(
    WellCMS::getPanel('app'), // Where `app` is the ID of the panel you want to test.
);
```
...
WellCMS includes a selection of helpers for testing tables. A full guide to testing tables can be found [in the Table Builder documentation](../tables/testing).
...
```php
use App\WellCMS\Resources\CategoryResource\Pages\EditCategory;
use function Pest\Livewire\livewire;
