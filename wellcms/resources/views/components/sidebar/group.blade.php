...
@php
    $sidebarCollapsible = $sidebarCollapsible && wellcms()->isSidebarCollapsibleOnDesktop();
...
<li
    ...
    {{
        $attributes->class([
            're-sidebar-group flex flex-col gap-y-1',
            're-active' => $active,
        ])
    }}
>
    @if ($label)
        <div
            ...
            @endif
            @class([
                're-sidebar-group-button flex items-center gap-x-3 px-2 py-2',
        ...
        >
            @if ($icon)
                <x-wellcms::icon
                    :icon="$icon"
                    class="re-sidebar-group-icon h-6 w-6 text-gray-400 dark:text-gray-500"
                />
            @endif

            <span
                class="re-sidebar-group-label flex-1 text-sm font-medium leading-6 text-gray-500 dark:text-gray-400"
            >
                {{ $label }}
            </span>
            ...
            @if ($collapsible)
                <x-wellcms::icon-button
                    ...
                    class="re-sidebar-group-collapse-button"
    ...
    @if ($hasDropdown)
        <x-wellcms::dropdown
            :placement="(__('wellcms-panels::layout.direction') === 'rtl') ? 'left-start' : 'right-start'"
            teleport
            x-show="! $store.sidebar.isOpen"
        >
            <x-slot name="trigger">
    ...
    <ul
        ...
        @if ($sidebarCollapsible)
            ...
            x-transition:enter-end="opacity-100"
        @endif
        class="re-sidebar-group-items flex flex-col gap-y-1"
    >
        @foreach ($items as $item)
            @php
