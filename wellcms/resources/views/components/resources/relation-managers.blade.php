...
<div class="re-resource-relation-managers flex flex-col gap-y-6">
    @php
        ...
        $normalizeRelationManagerClass = function (string | WellCMS\Resources\RelationManagers\RelationManagerConfiguration $manager): string {
            if ($manager instanceof \WellCMS\Resources\RelationManagers\RelationManagerConfiguration) {
        ...
        <x-wellcms::tabs>
            @php
                ...
                if ($content) {
                    match ($contentTabPosition) {
                        \WellCMS\Resources\Pages\ContentTabPosition::After => $tabs = array_merge($tabs, [null => null]),
            ...
            @foreach ($tabs as $tabKey => $manager)
                @php
                    ...
                    $isGroup = $manager instanceof \WellCMS\Resources\RelationManagers\RelationGroup;
    ...
    @if (filled($activeManager) && isset($managers[$activeManager]))
        ...
        >
            ...
            @if ($managers[$activeManager] instanceof \WellCMS\Resources\RelationManagers\RelationGroup)
                @foreach ($managers[$activeManager]->ownerRecord($ownerRecord)->pageClass($pageClass)->getManagers() as $groupedManagerKey => $groupedManager)
                    ...
                    @livewire(
                        ...
                        [...$managerLivewireProperties, ...(($groupedManager instanceof \WellCMS\Resources\RelationManagers\RelationManagerConfiguration) ? [...$groupedManager->relationManager::getDefaultProperties(), ...$groupedManager->getProperties()] : $groupedManager::getDefaultProperties())],
            ...
            @else
                ...
                @livewire(
                    ...
                    [...$managerLivewireProperties, ...(($manager instanceof \WellCMS\Resources\RelationManagers\RelationManagerConfiguration) ? [...$manager->relationManager::getDefaultProperties(), ...$manager->getProperties()] : $manager::getDefaultProperties())],
