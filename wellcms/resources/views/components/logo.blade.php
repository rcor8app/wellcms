@php
    $brandName = wellcms()->getBrandName();
    $brandLogo = wellcms()->getBrandLogo();
    $brandLogoHeight = wellcms()->getBrandLogoHeight() ?? '1.5rem';
    $darkModeBrandLogo = wellcms()->getDarkModeBrandLogo();
...
@capture($content, $logo, $isDarkMode = false)
    ...
    @elseif (filled($logo))
        <img
            alt="{{ __('wellcms-panels::layout.logo.alt', ['name' => $brandName]) }}"
            src="{{ $logo }}"
            {{
                $attributes
                    ->class([$getLogoClasses($isDarkMode)])
