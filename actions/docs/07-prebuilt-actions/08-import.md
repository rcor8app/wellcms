...
```php
use App\WellCMS\Imports\ProductImporter;
use WellCMS\Actions\ImportAction;

ImportAction::make()
    ->importer(ProductImporter::class)
```

If you want to add this action to the header of a table instead, you can use `WellCMS\Tables\Actions\ImportAction`:

```php
use App\WellCMS\Imports\ProductImporter;
use WellCMS\Tables\Actions\ImportAction;
...
The ["importer" class needs to be created](#creating-an-importer) to tell WellCMS how to import each row of the CSV.
...
To create an importer class for a model, you may use the `make:wellcms-importer` command, passing the name of a model:

```bash
php artisan make:wellcms-importer Product
```

This will create a new class in the `app/WellCMS/Imports` directory. You now need to define the [columns](#defining-importer-columns) that can be imported.

### Automatically generating importer columns

If you'd like to save time, WellCMS can automatically generate the [columns](#defining-importer-columns) for you, based on your model's database columns, using `--generate`:

```bash
php artisan make:wellcms-importer Product --generate
```
...
```php
use WellCMS\Actions\Imports\ImportColumn;
...
WellCMS also ships with some built-in casting methods:
...
If the column has a value, but the author cannot be found, the import will fail validation. WellCMS automatically adds validation to all relationship columns, to ensure that the relationship is not empty when it is required.
...
```php
use App\Models\Product;
use WellCMS\Actions\Imports\Exceptions\RowImportFailedException;
...
By default, WellCMS will attempt to "guess" which columns in the CSV match which columns in the database, to save the user time. It does this by attempting to find different combinations of the column name, with spaces, `-`, `_`, all cases insensitively. However, if you'd like to improve the guesses, you can call the `guess()` method with more examples of the column name that could be present in the CSV:
...
```php
use WellCMS\Actions\Imports\Models\Import;
...
The default job for processing imports is `WellCMS\Actions\Imports\Jobs\ImportCsv`. If you want to extend this class and override any of its methods, you may replace the original class in the `register()` method of a service provider:

```php
use App\Jobs\ImportCsv;
use WellCMS\Actions\Imports\Jobs\ImportCsv as BaseImportCsv;
...
```php
use WellCMS\Actions\Imports\Importer;
