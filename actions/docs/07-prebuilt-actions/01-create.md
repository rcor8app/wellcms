...
WellCMS includes a prebuilt action that is able to create Eloquent records. When the trigger button is clicked, a modal will open with a form inside. The user fills the form, and that data is validated and saved into the database. You may use it like so:
...
If you want to add this action to the header of a table instead, you can use `WellCMS\Tables\Actions\CreateAction`:

```php
use WellCMS\Forms\Components\TextInput;
use WellCMS\Tables\Actions\CreateAction;
