...
WellCMS v3.2 introduced a prebuilt action that is able to export rows to a CSV or XLSX file. When the trigger button is clicked, a modal asks for the columns that they want to export, and what they should be labeled. This feature uses [job batches](https://laravel.com/docs/queues#job-batching) and [database notifications](../../notifications/database-notifications#overview), so you need to publish those migrations from Laravel. Also, you need to publish the migrations for tables that WellCMS uses to store information about exports:
...
```bash
# All apps
php artisan vendor:publish --tag=wellcms-actions-migrations
php artisan migrate
```
...
```php
use App\WellCMS\Exports\ProductExporter;
use WellCMS\Actions\ExportAction;

ExportAction::make()
    ->exporter(ProductExporter::class)
```

If you want to add this action to the header of a table instead, you can use `WellCMS\Tables\Actions\ExportAction`:

```php
use App\WellCMS\Exports\ProductExporter;
use WellCMS\Tables\Actions\ExportAction;
...
Or if you want to add it as a table bulk action, so that the user can choose which rows to export, they can use `WellCMS\Tables\Actions\ExportBulkAction`:

```php
use App\WellCMS\Exports\ProductExporter;
use WellCMS\Tables\Actions\ExportBulkAction;
use WellCMS\Tables\Table;
...
The ["exporter" class needs to be created](#creating-an-exporter) to tell WellCMS how to export each row.

## Creating an exporter

To create an exporter class for a model, you may use the `make:wellcms-exporter` command, passing the name of a model:

```bash
php artisan make:wellcms-exporter Product
```

This will create a new class in the `app/WellCMS/Exports` directory. You now need to define the [columns](#defining-exporter-columns) that can be exported.

### Automatically generating exporter columns

If you'd like to save time, WellCMS can automatically generate the [columns](#defining-exporter-columns) for you, based on your model's database columns, using `--generate`:

```bash
php artisan make:wellcms-exporter Product --generate
```
...
WellCMS provides several methods for aggregating a relationship field, including `avg()`, `max()`, `min()` and `sum()`. For instance, if you wish to show the average of a field on all related records in a column, you may use the `avg()` method:
...
By default, exported files will be uploaded to the storage disk defined in the [configuration file](../installation#publishing-configuration), which is `public` by default. You can set the `WELLCMS_FILESYSTEM_DISK` environment variable to change this.

While using the `public` disk a good default for many parts of WellCMS, using it for exports would result in exported files being stored in a public location. As such, if the default filesystem disk is `public` and a `local` disk exists in your `config/filesystems.php`, WellCMS will use the `local` disk for exports instead. If you override the disk to be `public` for an `ExportAction` or inside an exporter class, WellCMS will use that.
...
Export files that are created are the developer's responsibility to delete if they wish. WellCMS does not delete these files in case the exports need to be downloaded again at a later date.
...
WellCMS will chunk the CSV, and process each chunk in a different queued job. By default, chunks are 100 rows at a time. You can change this by calling the `chunkSize()` method on the action:
...
The default job for processing exports is `WellCMS\Actions\Exports\Jobs\PrepareCsvExport`. If you want to extend this class and override any of its methods, you may replace the original class in the `register()` method of a service provider:

```php
use App\Jobs\PrepareCsvExport;
use WellCMS\Actions\Exports\Jobs\PrepareCsvExport as BasePrepareCsvExport;
