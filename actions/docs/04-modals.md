...
Action::make('advance')
    ...
    ->modalContent(view('wellcms.pages.actions.advance'))
```
...
Action::make('advance')
    ->action(fn (Post $record) => $record->advance())
    ->modalContentFooter(view('wellcms.pages.actions.advance'))
```
...
```php
use WellCMS\Support\View\Components\Modal;
...
When you use database queries or other heavy operations inside modal configuration methods like `modalHeading()`, they can be executed more than once. This is because WellCMS uses these methods to decide whether to render the modal or not, and also to render the modal's content.

To skip the check that WellCMS does to decide whether to render the modal, you can use the `modal()` method, which will inform WellCMS that the modal exists for this action and it does not need to check again:
...
Action::make('create')
    ...
    ->modalContent(view('wellcms.pages.actions.create'))
```
