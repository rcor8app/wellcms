...
use WellCMS\Actions\Exports\Jobs\CreateXlsxFile;
use WellCMS\Actions\Exports\Jobs\ExportCompletion;
use WellCMS\Actions\Exports\Jobs\PrepareCsvExport;
...
use WellCMS\Tables\Actions\ExportAction as ExportTableAction;
use WellCMS\Tables\Actions\ExportBulkAction as ExportTableBulkAction;
...
trait CanExportRecords
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (ExportAction | ExportTableAction | ExportTableBulkAction $action): string => __('wellcms-actions::export.label', ['label' => $action->getPluralModelLabel()]));

        $this->modalHeading(fn (ExportAction | ExportTableAction | ExportTableBulkAction $action): string => __('wellcms-actions::export.modal.heading', ['label' => $action->getPluralModelLabel()]));

        $this->modalSubmitActionLabel(__('wellcms-actions::export.modal.actions.export.label'));

        $this->groupedIcon(WellCMSIcon::resolve('actions::export-action.grouped') ?? 'heroicon-m-arrow-down-tray');

        $this->form(fn (ExportAction | ExportTableAction | ExportTableBulkAction $action): array => [
            ...($action->hasColumnMapping() ? [Fieldset::make(__('wellcms-actions::export.modal.form.columns.label'))
                ->columns(1)
                ->inlineLabel()
                    ...
                    return array_map(
                        fn (ExportColumn $column): Split => Split::make([
                            Forms\Components\Checkbox::make('isEnabled')
                                ->label(__('wellcms-actions::export.modal.form.columns.form.is_enabled.label', ['column' => $column->getName()]))
                                ->hiddenLabel()
                                ->default($column->isEnabledByDefault())
                                ->live()
                                ->grow(false),
                            Forms\Components\TextInput::make('label')
                                ->label(__('wellcms-actions::export.modal.form.columns.form.label.label', ['column' => $column->getName()]))
                                ->hiddenLabel()
        ...
        $this->action(function (ExportAction | ExportTableAction | ExportTableBulkAction $action, array $data, Component $livewire) {
            ...
            if ($maxRows < $totalRows) {
                Notification::make()
                    ->title(__('wellcms-actions::export.notifications.max_rows.title'))
                    ->body(trans_choice('wellcms-actions::export.notifications.max_rows.body', $maxRows, [
            ...
            if (
                ...
                Notification::make()
                    ...
                    ->body(trans_choice('wellcms-actions::export.notifications.started.body', $export->total_rows, [
        ...
        $this->successNotificationTitle(__('wellcms-actions::export.notifications.started.title'));
