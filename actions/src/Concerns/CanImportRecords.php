...
use WellCMS\Actions\Imports\Events\ImportCompleted;
use WellCMS\Actions\Imports\Events\ImportStarted;
use WellCMS\Actions\Imports\ImportColumn;
use WellCMS\Actions\Imports\Importer;
use WellCMS\Actions\Imports\Jobs\ImportCsv;
...
use WellCMS\Support\ChunkIterator;
use WellCMS\Support\Facades\WellCMSIcon;
use WellCMS\Tables\Actions\Action as TableAction;
use WellCMS\Tables\Actions\ImportAction as ImportTableAction;
...
trait CanImportRecords
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (ImportAction | ImportTableAction $action): string => __('wellcms-actions::import.label', ['label' => $action->getPluralModelLabel()]));

        $this->modalHeading(fn (ImportAction | ImportTableAction $action): string => __('wellcms-actions::import.modal.heading', ['label' => $action->getPluralModelLabel()]));

        $this->modalDescription(fn (ImportAction | ImportTableAction $action): Htmlable => $action->getModalAction('downloadExample'));

        $this->modalSubmitActionLabel(__('wellcms-actions::import.modal.actions.import.label'));

        $this->groupedIcon(WellCMSIcon::resolve('actions::import-action.grouped') ?? 'heroicon-m-arrow-up-tray');

        $this->form(fn (ImportAction | ImportTableAction $action): array => array_merge([
            FileUpload::make('file')
                ->label(__('wellcms-actions::import.modal.form.file.label'))
                ->placeholder(__('wellcms-actions::import.modal.form.file.placeholder'))
                ...
                ->required()
                ->hiddenLabel(),
            Fieldset::make(__('wellcms-actions::import.modal.form.columns.label'))
                ->columns(1)
                ->inlineLabel()
        ...
        $this->action(function (ImportAction | ImportTableAction $action, array $data) {
            ...
            if ($maxRows < $totalRows) {
                Notification::make()
                    ->title(__('wellcms-actions::import.notifications.max_rows.title'))
                    ->body(trans_choice('wellcms-actions::import.notifications.max_rows.body', $maxRows, [
                        ...
                        ->when(
                            ...
                            fn (Notification $notification) => $notification->actions([
                                NotificationAction::make('downloadFailedRowsCsv')
                                    ->label(trans_choice('wellcms-actions::import.notifications.completed.actions.download_failed_rows_csv.label', $failedRowsCount, [
                                        'count' => Number::format($failedRowsCount),
                                    ]))
                                    ->color('danger')
                                    ->url(route('wellcms.imports.failed-rows.download', ['import' => $import], absolute: false), shouldOpenInNewTab: true)
                                    ->markAsRead(),
                            ]),
                        )
                        ->when(
            ...
            if (
                ...
                Notification::make()
                    ...
                    ->body(trans_choice('wellcms-actions::import.notifications.started.body', $import->total_rows, [
        ...
        $this->registerModalActions([
            (match (true) {
                ...
                ->label(__('wellcms-actions::import.modal.actions.download_example.label'))
                ->link()
                ->action(function (): StreamedResponse {
                    ...
                    return response()->streamDownload(function () use ($csv) {
                        echo $csv->toString();
                    }, __('wellcms-actions::import.example_csv.file_name', ['importer' => (string) str($this->getImporter())->classBasename()->kebab()]), [
        ...
        $this->successNotificationTitle(__('wellcms-actions::import.notifications.started.title'));
    ...
    public function getFileValidationRules(): array
    {
        $fileRules = [
            ...
            fn (): Closure => function (string $attribute, mixed $value, Closure $fail) {
                ...
                $fail(trans_choice('wellcms-actions::import.modal.form.file.rules.duplicate_columns', count($filledDuplicateCsvColumns), [
