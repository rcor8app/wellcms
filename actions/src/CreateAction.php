...
class CreateAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (): string => __('wellcms-actions::create.single.label', ['label' => $this->getModelLabel()]));

        $this->modalHeading(fn (): string => __('wellcms-actions::create.single.modal.heading', ['label' => $this->getModelLabel()]));

        $this->modalSubmitActionLabel(__('wellcms-actions::create.single.modal.actions.create.label'));
        ...
        $this->extraModalFooterActions(function (): array {
            return $this->canCreateAnother() ? [
                $this->makeModalSubmitAction('createAnother', arguments: ['another' => true])
                    ->label(__('wellcms-actions::create.single.modal.actions.create_another.label')),
            ] : [];
        });

        $this->successNotificationTitle(__('wellcms-actions::create.single.notifications.created.title'));

        $this->groupedIcon(WellCMSIcon::resolve('actions::create-action.grouped') ?? 'heroicon-m-plus');
