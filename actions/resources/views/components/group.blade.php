...
@if (! ($dynamicComponent && $group))
    @php
        $group = \WellCMS\Actions\ActionGroup::make($actions)
...
@else
    @php
        ...
        foreach ($group->getActions() as $action) {
            ...
            if ($action instanceof \WellCMS\Actions\ActionGroup && (! $action->hasDropdown())) {
    ...
    >
        <x-slot name="trigger">
            <x-dynamic-component
                ...
                :attributes="\WellCMS\Support\prepare_inherited_attributes($attributes)->merge($group->getExtraAttributes(), escape: false)"
            >
                {{ $slot }}
            </x-dynamic-component>
        </x-slot>
