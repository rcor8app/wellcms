@php
    ...
    use WellCMS\Tables\Columns\TextColumn\TextColumnSize;
    ...
    if (is_array($arrayState)) {
        ...
        if ((! $isListWithLineBreaks) && (! $isBadge)) {
            $arrayState = implode(
                ...
                array_map(
                    function ($value) {
                        if ($value instanceof \WellCMS\Support\Contracts\HasLabel) {
                ...
                <{{ $isListWithLineBreaks ? 'li' : 'div' }}>
                    @if ($isLimitedListExpandable)
                        <x-wellcms::link
                            ...
                            x-show="isLimited"
                        >
                            {{ trans_choice('wellcms-tables::table.columns.text.actions.expand_list', $limitedArrayStateCount) }}
                        </x-wellcms::link>
                        ...
                        <x-wellcms::link
                            ...
                            x-show="! isLimited"
                        >
                            {{ trans_choice('wellcms-tables::table.columns.text.actions.collapse_list', $limitedArrayStateCount) }}
                        </x-wellcms::link>
                    @else
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            {{ trans_choice('wellcms-tables::table.columns.text.more_list_items', $limitedArrayStateCount) }}
                        </span>
                    @endif
                </{{ $isListWithLineBreaks ? 'li' : 'div' }}>
            @endif
