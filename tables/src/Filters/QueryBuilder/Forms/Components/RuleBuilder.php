...
namespace WellCMS\Tables\Filters\QueryBuilder\Forms\Components;

use Exception;
use WellCMS\Forms\ComponentContainer;
use WellCMS\Forms\Components\Actions\Action;
use WellCMS\Forms\Components\Builder;
use WellCMS\Forms\Components\Component;
use WellCMS\Forms\Components\Repeater;
use WellCMS\Tables\Filters\QueryBuilder\Concerns\HasConstraints;
use WellCMS\Tables\Filters\QueryBuilder\Constraints\Constraint;
...
class RuleBuilder extends Builder
{
    ...
    protected function setUp(): void
    {
        ...
        $this
                    ...
                    Builder\Block::make(static::OR_BLOCK_NAME)
                        ->label(function (?array $state, ?string $uuid) use ($component) {
                            if (blank($state) || blank($uuid)) {
                                return __('wellcms-tables::filters/query-builder.form.or_groups.block.label');
                            }

                            if (! count($state[static::OR_BLOCK_GROUPS_REPEATER_NAME] ?? [])) {
                                return __('wellcms-tables::filters/query-builder.no_rules');
                            }
                            ...
                            return '(' . $itemLabels->implode(') ' . __('wellcms-tables::filters/query-builder.form.or_groups.block.or') . ' (') . ')';
                        })
                        ->icon('heroicon-m-bars-4')
                        ->schema(fn (): array => [
                            Repeater::make(static::OR_BLOCK_GROUPS_REPEATER_NAME)
                                ->label(__('wellcms-tables::filters/query-builder.form.or_groups.label'))
                                ->schema(fn (): array => [
                                    static::make('rules')
                                ...
                                ->addAction(fn (Action $action) => $action
                                    ->label(__('wellcms-tables::filters/query-builder.actions.add_rule_group.label'))
                                    ->icon('heroicon-s-plus'))
                                ->labelBetweenItems(__('wellcms-tables::filters/query-builder.item_separators.or'))
                                ->collapsible()
                                ...
                                ->itemLabel(function (ComponentContainer $container, array $state): string {
                                    ...
                                    return '(' . $blockLabels->implode(') ' . __('wellcms-tables::filters/query-builder.form.rules.item.and') . ' (') . ')';
                                })
            ...
            ->addAction(fn (Action $action) => $action
                ->label(__('wellcms-tables::filters/query-builder.actions.add_rule.label'))
                ->icon('heroicon-s-plus'))
            ->addBetweenAction(fn (Action $action) => $action->hidden())
            ->label(__('wellcms-tables::filters/query-builder.form.rules.label'))
            ->hiddenLabel()
            ->labelBetweenItems(__('wellcms-tables::filters/query-builder.item_separators.and'))
            ->blockNumbers(false)
            ->collapsible()
            ->cloneable()
            ->reorderable(false)
