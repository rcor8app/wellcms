...
namespace WellCMS\Tables\Filters\QueryBuilder\Constraints\SelectConstraint\Operators;
...
class IsOperator extends Operator
{
    ...
    public function getLabel(): string
    {
        return __(
            $this->isInverse() ?
                'wellcms-tables::filters/query-builder.operators.select.is.label.inverse' :
                'wellcms-tables::filters/query-builder.operators.select.is.label.direct',
        );
    }
    ...
    public function getSummary(): string
    {
        ...
        $joinedValues = Arr::join(
            $labels,
            glue: __('wellcms-tables::filters/query-builder.operators.select.is.summary.values_glue.0'),
            finalGlue: __('wellcms-tables::filters/query-builder.operators.select.is.summary.values_glue.final'),
        );

        return __(
            $this->isInverse() ?
                'wellcms-tables::filters/query-builder.operators.select.is.summary.inverse' :
                'wellcms-tables::filters/query-builder.operators.select.is.summary.direct',
            [
    ...
    public function getFormSchema(): array
    {
        ...
        $field = Select::make($constraint->isMultiple() ? 'values' : 'value')
            ->label(__($constraint->isMultiple() ? 'wellcms-tables::filters/query-builder.operators.select.is.form.values.label' : 'wellcms-tables::filters/query-builder.operators.select.is.form.value.label'))
