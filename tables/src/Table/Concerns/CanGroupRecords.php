...
use WellCMS\Tables\Grouping\Group;
...
trait CanGroupRecords
{
    ...
    public function getGroupRecordsTriggerAction(): Action
    {
        $action = Action::make('groupRecords')
            ->label(__('wellcms-tables::table.actions.group.label'))
            ->iconButton()
            ->icon(WellCMSIcon::resolve('tables::actions.group') ?? 'heroicon-m-rectangle-stack')
            ->color('gray')
