...
trait CanReorderRecords
{
    ...
    public function getReorderRecordsTriggerAction(bool $isReordering): Action
    {
        $action = Action::make('reorderRecords')
            ->label($isReordering ? __('wellcms-tables::table.actions.disable_reordering.label') : __('wellcms-tables::table.actions.enable_reordering.label'))
            ->iconButton()
            ->icon($isReordering ? (WellCMSIcon::resolve('tables::actions.disable-reordering') ?? 'heroicon-m-check') : (WellCMSIcon::resolve('tables::actions.enable-reordering') ?? 'heroicon-m-arrows-up-down'))
            ->color('gray')
