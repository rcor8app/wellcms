...
class AttachAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(__('wellcms-actions::attach.single.label'));

        $this->modalHeading(fn (): string => __('wellcms-actions::attach.single.modal.heading', ['label' => $this->getModelLabel()]));

        $this->modalSubmitActionLabel(__('wellcms-actions::attach.single.modal.actions.attach.label'));
        ...
        $this->extraModalFooterActions(function (): array {
            return $this->canAttachAnother() ? [
                $this->makeModalSubmitAction('attachAnother', ['another' => true])
                    ->label(__('wellcms-actions::attach.single.modal.actions.attach_another.label')),
            ] : [];
        });

        $this->successNotificationTitle(__('wellcms-actions::attach.single.notifications.attached.title'));
    ...
    public function getRecordSelect(): Select
    {
                ...
                ->when(
                    ...
                    fn (Builder $query): Builder => $query->whereDoesntHave(
                        ...
                        fn (Builder $query): Builder => $query->where(
                            // https://github.com/wellcmsphp/wellcms/issues/8067
        ...
        $select = Select::make('recordId')
            ->label(__('wellcms-actions::attach.single.modal.fields.record_id.label'))
            ->required()
