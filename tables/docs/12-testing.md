...
it('links to the correct WellCMS sites', function () {
    ...
    livewire(PostResource\Pages\ListPosts::class)
        ->assertTableActionHasUrl('wellcms', 'https://wellcmsphp.com/', $post)
        ->assertTableActionDoesNotHaveUrl('wellcms', 'https://github.com/wellcmsphp/wellcms', $post)
        ->assertTableActionShouldOpenUrlInNewTab('wellcms', $post)
...
```php
use WellCMS\Tables\Columns\Summarizers\Average;
use WellCMS\Tables\Columns\TextColumn;
