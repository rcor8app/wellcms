...
namespace WellCMS\Widgets\Commands;
...
#[AsCommand(name: 'make:wellcms-widget')]
class MakeWidgetCommand extends Command
{
    use CanManipulateFiles;

    protected $description = 'Create a new WellCMS widget class';

    protected $signature = 'make:wellcms-widget {name?} {--R|resource=} {--C|chart} {--T|table} {--S|stats-overview} {--panel=} {--F|force}';
    ...
    public function handle(): int
    {
        ...
        if (! $panel) {
            ...
            $namespace = (count($widgetNamespaces) > 1) ?
                ...
                (Arr::first($widgetNamespaces) ?? 'App\\WellCMS\\Widgets');
            $path = (count($widgetDirectories) > 1) ?
                $widgetDirectories[array_search($namespace, $widgetNamespaces)] :
                (Arr::first($widgetDirectories) ?? app_path('WellCMS/Widgets/'));
        } else {
        ...
        $this->components->info("WellCMS widget [{$path}] created successfully.");
