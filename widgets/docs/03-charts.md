...
WellCMS comes with many "chart" widget templates, which you can use to display real-time, interactive charts.

Start by creating a widget with the command:

```bash
php artisan make:wellcms-widget BlogPostsChart --chart
```
...
The `getData()` method is used to return an array of datasets and labels. Each dataset is a labeled array of points to plot on the chart, and each label is a string. This structure is identical to the [Chart.js](https://www.chartjs.org/docs) library, which WellCMS uses to render charts. You may use the [Chart.js documentation](https://www.chartjs.org/docs) to fully understand the possibilities to return from `getData()`, based on the chart type.
...
use WellCMS\Widgets\ChartWidget;
...
To generate chart data from an Eloquent model, WellCMS recommends that you install the `flowframe/laravel-trend` package. You can view the [documentation](https://github.com/Flowframe/laravel-trend).
...
Create a new JavaScript file where you will define your custom plugin. In this guide, we'll call it `wellcms-chart-js-plugins.js`. Import the plugin, and add it to the `window.wellcmsChartJsPlugins` array:

```javascript
import ChartDataLabels from 'chartjs-plugin-datalabels'

window.wellcmsChartJsPlugins ??= []
window.wellcmsChartJsPlugins.push(ChartDataLabels)
```

It's important to initialise the array if it has not been already, before pushing onto it. This ensures that mutliple JavaScript files (especially those from WellCMS plugins) that register Chart.js plugins do not overwrite each other, regardless of the order they are booted in.

You can push as many plugins to the `wellcmsChartJsPlugins` array as you would like to install, you do not need a separate file to import each plugin.
    ...
    plugins: [
        laravel({
            input: [
                ...
                'resources/js/app.js',
                'resources/css/wellcms/admin/theme.css',
                'resources/js/wellcms-chart-js-plugins.js', // Include the new file in the `input` array so it is built
            ],
        }),
    ],
});
...
### Step 4: Register the JavaScript file in WellCMS

WellCMS needs to know to include this JavaScript file when rendering chart widgets. You can do this in the `boot()` method of a service provider like `AppServiceProvider`:
...
WellCMSAsset::register([
    Js::make('chart-js-plugins', Vite::asset('resources/js/wellcms-chart-js-plugins.js'))->module(),
]);
```
