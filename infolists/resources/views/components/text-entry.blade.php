@php
    use WellCMS\Infolists\Components\TextEntry\TextEntrySize;
    use WellCMS\Support\Enums\Alignment;
    use WellCMS\Support\Enums\FontFamily;
        ...
        if (is_array($arrayState)) {
            ...
            if ((! $isListWithLineBreaks) && (! $isBadge)) {
                $arrayState = implode(
                    ', ',
                    array_map(
                        fn ($value) => $value instanceof \WellCMS\Support\Contracts\HasLabel ? $value->getLabel() : $value,
                        $arrayState,
                    ),
                );
            }
                        ...
                        <{{ $isListWithLineBreaks ? 'li' : 'div' }}>
                            @if ($isLimitedListExpandable)
                                <x-wellcms::link
                                    ...
                                    x-show="isLimited"
                                >
                                    {{ trans_choice('wellcms-infolists::components.entries.text.actions.expand_list', $limitedArrayStateCount) }}
                                </x-wellcms::link>
                                ...
                                <x-wellcms::link
                                    ...
                                    x-show="! isLimited"
                                >
                                    {{ trans_choice('wellcms-infolists::components.entries.text.actions.collapse_list', $limitedArrayStateCount) }}
                                </x-wellcms::link>
                            @else
                                <span
                                    class="text-sm text-gray-500 dark:text-gray-400"
                                >
                                    {{ trans_choice('wellcms-infolists::components.entries.text.more_list_items', $limitedArrayStateCount) }}
                                </span>
                            @endif
                        </{{ $isListWithLineBreaks ? 'li' : 'div' }}>
                    @endif
