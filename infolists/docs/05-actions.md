...
WellCMS's infolists can use [Actions](../actions). They are buttons that can be added to any infolist component. Also, you can [render anonymous sets of actions](#adding-anonymous-actions-to-an-infolist-without-attaching-them-to-a-component) on their own, that are not attached to a particular infolist component.

## Defining a infolist component action

Action objects inside an infolist component are instances of `WellCMS/Infolists/Components/Actions/Action`. You must pass a unique name to the action's `make()` method, which is used to identify it amongst others internally within WellCMS. You can [customize the trigger button](../actions/trigger-button) of an action, and even [open a modal](../actions/modals) with little effort:
...
use WellCMS\Infolists\Components\Actions\Action;
use WellCMS\Infolists\Components\TextEntry;
...
You may pass multiple affix actions to an entry by passing them in an array to either `prefixActions()` or `suffixActions()`. Either method can be used, or both at once, WellCMS will render all the registered actions in order:
...
You may pass multiple hint actions to a entry by passing them in an array to `hintActions()`. WellCMS will render all the registered actions in order:
...
use WellCMS\Infolists\Components\Actions\Action;
use WellCMS\Infolists\Components\ViewEntry;
use WellCMS\Infolists\Set;

ViewEntry::make('status')
    ->view('wellcms.infolists.entries.status-switcher')
    ->registerActions([
...
use App\Actions\ResetStars;
use WellCMS\Infolists\Components\Actions;
use WellCMS\Infolists\Components\Actions\Action;
